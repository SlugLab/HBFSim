...
struct CombinedConfig {
    bool enabled = false;
    bool valid = false;
    std::string source[2], stage[2], map_sha[2], stage_sha[2];
};
const CombinedConfig &combined_config() {
    static const CombinedConfig config = [] {
        CombinedConfig c;
        const char *flag = std::getenv("HBFSIM_QKV_COMBINED_V1");
        c.enabled = flag && std::strcmp(flag,"1") == 0;
        if (!c.enabled) return c;
        const char *target = std::getenv("HBFSIM_QKV_TARGET_KIND");
        if (target && *target) return c; // legacy process-global target conflicts
        const char *keys[2][4] = {
            {"HBFSIM_LI6_SOURCE_PTX_PATH","HBFSIM_LI6_STAGED_PTX_PATH",
             "HBFSIM_LI6_ABI_MAP_SHA256","HBFSIM_LI6_STAGED_PTX_SHA256"},
            {"HBFSIM_LI7_SOURCE_PTX_PATH","HBFSIM_LI7_STAGED_PTX_PATH",
             "HBFSIM_LI7_ABI_MAP_SHA256","HBFSIM_LI7_STAGED_PTX_SHA256"}};
        for (int i=0;i<2;++i) {
            std::string *values[4] = {&c.source[i],&c.stage[i],
                                      &c.map_sha[i],&c.stage_sha[i]};
            for (int j=0;j<4;++j) {
                const char *value = std::getenv(keys[i][j]);
                if (!value || !*value) return c;
                *values[j] = value;
            }
        }
        if (c.map_sha[0] != kQkvAbiMapSha ||
            c.map_sha[1] != kLi7AbiMapSha ||
            c.stage_sha[0] != kQkvStagedPtxSha ||
            c.stage_sha[1] != kLi7StagedPtxSha ||
            c.source[0] == c.source[1] || c.stage[0] == c.stage[1])
            return c;
        c.valid = true;
        return c;
    }();
...
bool combined_mode() { return combined_config().enabled; }
enum class ExactProfile { Li6, Li7, Invalid };
const char *profile_name(ExactProfile p) {
    return p == ExactProfile::Li6 ? "li6" :
           p == ExactProfile::Li7 ? "li7" : "invalid";
}
const char *profile_symbol(ExactProfile p) {
    return p == ExactProfile::Li6 ? kQkvName :
           p == ExactProfile::Li7 ? kLi7Name : nullptr;
}
const char *profile_map_sha(ExactProfile p) {
    return p == ExactProfile::Li6 ? kQkvAbiMapSha : kLi7AbiMapSha;
}
const char *profile_stage_sha(ExactProfile p) {
    return p == ExactProfile::Li6 ? kQkvStagedPtxSha : kLi7StagedPtxSha;
}
const char *profile_source_sha(ExactProfile p) {
    return p == ExactProfile::Li6 ? kQkvSourcePtxSha : kLi7SourcePtxSha;
}
ExactProfile legacy_profile() {
    return li7_target() ? ExactProfile::Li7 : ExactProfile::Li6;
}
ExactProfile profile_from_name(const char *name) {
    if (name && std::strcmp(name, kQkvName) == 0) return ExactProfile::Li6;
    if (name && std::strcmp(name, kLi7Name) == 0) return ExactProfile::Li7;
    return ExactProfile::Invalid;
}
...
QkvBoundIdentity &qkv_bound(ExactProfile p) {
    static QkvBoundIdentity values[2];
    return values[p == ExactProfile::Li7 ? 1 : 0];
}
...
bool li6_representative_envelope(ExactProfile profile, std::size_t bytes,
                                 unsigned int grid_x) {
    if (profile == ExactProfile::Li7) return bytes == 262144 && grid_x == 16;
    if (combined_mode())
        return (bytes == 25165824 && grid_x == 1536) ||
               (bytes == 8388608 && grid_x == 512) ||
...
const char *profile_env(ExactProfile p, const char *li6, const char *li7,
                        const char *legacy) {
    if (!combined_mode()) return std::getenv(legacy);
    const auto &c = combined_config();
    if (!c.valid || p == ExactProfile::Invalid) return nullptr;
    const int i = p == ExactProfile::Li6 ? 0 : 1;
    if (std::strcmp(li6,"HBFSIM_LI6_SOURCE_PTX_PATH") == 0) return c.source[i].c_str();
    if (std::strcmp(li6,"HBFSIM_LI6_STAGED_PTX_PATH") == 0) return c.stage[i].c_str();
    if (std::strcmp(li6,"HBFSIM_LI6_ABI_MAP_SHA256") == 0) return c.map_sha[i].c_str();
    if (std::strcmp(li6,"HBFSIM_LI6_STAGED_PTX_SHA256") == 0) return c.stage_sha[i].c_str();
    (void)li7;
    return nullptr;
}
bool qkv_config_pinned(ExactProfile p) {
    if (p == ExactProfile::Invalid) return false;
    if (combined_mode() && !combined_config().valid) return false;
    const char *map = profile_env(p,"HBFSIM_LI6_ABI_MAP_SHA256",
        "HBFSIM_LI7_ABI_MAP_SHA256","HBFSIM_QKV_ABI_MAP_SHA256");
    const char *stage = profile_env(p,"HBFSIM_LI6_STAGED_PTX_SHA256",
        "HBFSIM_LI7_STAGED_PTX_SHA256","HBFSIM_QKV_STAGED_PTX_SHA256");
    return map && stage && std::strcmp(map, profile_map_sha(p)) == 0 &&
           std::strcmp(stage, profile_stage_sha(p)) == 0;
}
...
using qkv_entry_query_type = int (*)(std::uint64_t, std::uint64_t,
                                     const char*, QkvLiveIdentityV1*);
qkv_entry_query_type qkv_entry_query() {
    return reinterpret_cast<qkv_entry_query_type>(dlsym(
        RTLD_DEFAULT, "hbfsim_qkv_live_identity_for_entry_v1"));
}
...
bool qkv_live_match(CUfunction original, CUcontext context,
                    ExactProfile profile, std::uint64_t *token,
                    std::uint64_t *module = nullptr) {
    if (!original || !context || !token || profile == ExactProfile::Invalid)
        return false;
    QkvLiveIdentityV1 identity{};
    identity.struct_size = sizeof(identity);
    int rc = 0;
    if (combined_mode()) {
        auto query = qkv_entry_query();
        if (!query) return false;
        rc = query(reinterpret_cast<std::uint64_t>(original),
                   reinterpret_cast<std::uint64_t>(context),
                   profile_symbol(profile), &identity);
    } else {
        auto query = qkv_identity_query();
        if (!query || profile != legacy_profile()) return false;
        rc = query(reinterpret_cast<std::uint64_t>(original),
                   reinterpret_cast<std::uint64_t>(context), &identity);
    }
    if (rc != 1 ||
    ...
    *token = identity.association_token;
    if (module) *module = identity.module;
    return true;
}
ExactProfile profile_for_function(CUfunction original, CUcontext context) {
    if (!original || !context) return ExactProfile::Invalid;
    ...
    const char *name = nullptr;
    if (get_name && get_name(&name, original) == CUDA_SUCCESS && name) {
        const auto p = profile_from_name(name);
        return combined_mode() || p == legacy_profile() ? p :
               ExactProfile::Invalid;
    }
    std::uint64_t token = 0;
    if (combined_mode()) {
        const bool li6 = qkv_live_match(original,context,ExactProfile::Li6,&token);
        const bool li7 = qkv_live_match(original,context,ExactProfile::Li7,&token);
        return li6 == li7 ? ExactProfile::Invalid :
               (li6 ? ExactProfile::Li6 : ExactProfile::Li7);
    }
    return qkv_live_match(original,context,legacy_profile(),&token) ?
           legacy_profile() : ExactProfile::Invalid;
}
...
bool qkv_bind_first_selected(CUfunction original, ExactProfile profile) {
    const char *path = profile_env(profile,"HBFSIM_LI6_SOURCE_PTX_PATH",
        "HBFSIM_LI7_SOURCE_PTX_PATH","HBFSIM_QKV_SOURCE_PTX_PATH");
    const char *stage_path = profile_env(profile,"HBFSIM_LI6_STAGED_PTX_PATH",
        "HBFSIM_LI7_STAGED_PTX_PATH","HBFSIM_QKV_STAGED_PTX_PATH");
    ...
    if (sha256(ptx.data(), ptx.size()) != profile_source_sha(profile))
        return false;
    ...
    if (!stage_input.read(staged.data(), stage_bytes) ||
        sha256(staged.data(), staged.size()) != profile_stage_sha(profile))
        return false;
    ...
    return bind && bind(original, ptx.data(), ptx.size(), profile_symbol(profile)) == 0 &&
...
struct QkvDecisionContext {
    ...
    ExactProfile profile = ExactProfile::Invalid;
...
std::atomic<std::uint64_t> qkv_decision_ordinal{1};
    ...
    if (path && *path) {
        ...
        if (FILE *out = std::fopen(path, "a")) {
            ...
            std::fprintf(out,
                ...
                "\"cuda_result\":%d,\"profile\":\"%s\","
                "\"selected_base\":\"0x%llx\",\"selected_bytes\":%zu,"
                "\"os_thread_id\":%ld,\"call_ordinal\":%llu}\n",
                ...
                static_cast<int>(result), profile_name(c.profile),
                static_cast<unsigned long long>(qkv_selected().base),
                qkv_selected().bytes,
                static_cast<long>(syscall(SYS_gettid)),
                static_cast<unsigned long long>(c.call_ordinal));
            std::fclose(out);
        }
    }
    return result;
...
static bool qkv_optin_exact_original(CUfunction func) {
    if (!qkv_enabled() || !func) return false;
    {
        ...
        if (qkv_bound(ExactProfile::Li6).original == func ||
            qkv_bound(ExactProfile::Li7).original == func) return true;
    }
    ...
    if (get_name && get_name(&name, func) == CUDA_SUCCESS && name) {
        const auto p = profile_from_name(name);
        if (p != ExactProfile::Invalid &&
            (combined_mode() || p == legacy_profile())) return true;
    }
    CUcontext context = nullptr;
    std::uint64_t token = 0;
    return cuCtxGetCurrent(&context) == CUDA_SUCCESS && context &&
           profile_for_function(func,context) != ExactProfile::Invalid;
}
...
bpftime_nv_qkv_pin_bound_identity_v1(CUfunction original) {
    if (!qkv_enabled() || !original) return -1;
    CUcontext context = nullptr;
    if (cuCtxGetCurrent(&context) != CUDA_SUCCESS || !context) return -2;
    const auto profile = profile_for_function(original, context);
    if (!qkv_config_pinned(profile)) return -1;
    std::uint64_t token = 0;
    std::uint64_t module = 0;
    if (!qkv_live_match(original, context, profile, &token, &module)) return -3;
    auto &state = nv_attach_get_hook_state();
    auto *impl = state.active_impl.load(std::memory_order_acquire);
    ...
    if (!patched || !*patched || !qkv_metadata_match(original, *patched))
        return -5;
    std::lock_guard<std::mutex> guard(qkv_bound_mu());
    qkv_bound(profile) = {original, *patched, context, token, module};
    return 0;
}
...
bpftime_nv_qkv_is_exact_original_v1(CUfunction original) {
    ...
    {
        std::lock_guard<std::mutex> guard(qkv_bound_mu());
        if (qkv_bound(ExactProfile::Li6).original == original ||
            qkv_bound(ExactProfile::Li7).original == original) return 1;
    }
    CUcontext context = nullptr;
    std::uint64_t token = 0;
    if (cuCtxGetCurrent(&context) != CUDA_SUCCESS || !context) return 0;
    const auto profile = profile_for_function(original, context);
    return qkv_live_match(original, context, profile, &token) ? 1 : 0;
}
...
bpftime_nv_qkv_exact_aggregate_launch_v1(
    ...
    qkv_decision_context().call_ordinal =
        qkv_decision_ordinal.fetch_add(1, std::memory_order_relaxed);
    ...
    const auto profile = profile_for_function(original, context);
    qkv_decision_context().profile = profile;
    ...
    if (!qkv_config_pinned(profile) ||
        !(strict_launch_bridge_enabled() ||
          (policy && std::strcmp(policy, "partial") == 0)))
        return qkv_record_decision("REJECTED", original,
                                   CUDA_ERROR_NOT_SUPPORTED, "config_or_strict_mode");
    if (!li6_representative_envelope(profile, selected.bytes, grid_x) || grid_y != 1 ||
        grid_z != 1 || block_x != (profile == ExactProfile::Li7 ? 32u : 16u) || block_y != 4 ||
        block_z != 1 || shared_bytes != (profile == ExactProfile::Li7 ? 528u : 272u))
        return qkv_record_decision("REJECTED", original,
                                   CUDA_ERROR_NOT_SUPPORTED, "target_envelope");
    std::uint64_t token = 0;
    std::uint64_t module = 0;
    if (!qkv_live_match(original, context, profile, &token, &module))
        return qkv_record_decision("REJECTED", original,
    ...
    {
        std::lock_guard<std::mutex> guard(qkv_bound_mu());
        bound = qkv_bound(profile);
    }
    if (!bound.original) {
        if (!qkv_bind_first_selected(original, profile))
            return qkv_record_decision("REJECTED", original,
    ...
    if (bound.original != original || bound.context != context ||
        bound.association_token != token || bound.module != module || !bound.patched)
        return qkv_record_decision("REJECTED", original,
