...
| [prefetch_replay.py](../../scripts/eval/prefetch_replay.py) | `validate_nodes`, `replay` | Preserve complete compute-only batches; the explicit HF route-horizon path instead replays 335 route gaps and retains terminal demand without issue/consume, then drains only for conservation; shared service, capacity, LRU and censoring remain projected |
| [run_prefetch.py](../../scripts/eval/run_prefetch.py) | `prepare`, `run` | Bind immutable inventory/budget/routes plus the selected compute-only or HF route-horizon contract, run three fresh native service policies and reconcile traffic; requested-rho and latency controls remain PROJECTED, not generation or GPU timing |
...
| [hbf_dependent_delay.cu](../../benchmarks/cuda/hbf_dependent_delay.cu) | `hbf_dependent_delay`, `options`, `main` | K-hop single-lane-per-warp dependency and trusted transform/load/config; explicit ABBA-only diagnostic-blocks=1 changes host geometry while the default stays SM-scaled and device kernel/helper images stay unchanged |
...
| [verify_hf_metadata.py](../../scripts/eval/verify_hf_metadata.py) | `verify`, `validate_refresh`, `check_current_inputs` | Read only approved metadata extents, reconcile complete HF tensors with the frozen historical donor, validate immutable bundles before final publication, and recheck current file identity without authenticating weight payloads |
| [verify_hf_metadata.py](../../scripts/eval/verify_hf_metadata.py) | `validate_receipt_contract`, `validate_frozen_payloads` | Share the exact metadata-only claim and artifact/input/tensor/observation reconciliation between disk bundles and immutable consumer buffers |
| [evaluation_inventory.py](../../scripts/eval/evaluation_inventory.py) | `load_hf_snapshot`, `adapt_hf_inventory`, `validate_evaluation_inventory`, `publish_inventory` | Derive a bounded HF BF16 inventory from published frozen evidence; retain shard-local extents, explicit KV head dimensions and MOCK attribution; refuse cross-format identities and output aliases without reopening weights |
| [budget_fast_tier.py](../../scripts/eval/budget_fast_tier.py) | HF `budget_fast_tier` path | Recompute HF deductions from the same snapshot and exact inventory bytes; preserve distinct canonical/file hashes and whole-expert rho semantics |
| [timing_future_abi.hpp](../../include/hbfsim/timing_future_abi.hpp) | `poll_state`, `consume_state`, `commit_transition`, `accounting_delta` | Versioned kernel-local 64/32-byte state and metadata, per-lane transitions and trace-before-accounting settlement; complete capability only in explicit ON/CUDA13/sm120 builds |
| [context.cpp](../../src/cuda_runtime/context.cpp) | `launch_gate_activate` | Derive v4 FAST-scalar capability from actual control/profile; retain mapping on failed activation with a retained generation |
| [timing_binding.cpp](../../src/cuda_runtime/timing_binding.cpp) | `add_future_module`, `reject_future_module`, `activate_with_capabilities`, `invalidate` | Bind exact owner/domain/generation and retain rejected-module classification/quarantine; retire generation-zero future configurations |
| [launch_gate.cpp](../../src/cuda_runtime/launch_gate.cpp) | `read_future_requirements`, `initialize_future_control`, `require_timing_binding`, `future_manifest_match` | Verify actual image contracts, checked disabled-first/enable-last publication and exact selected kernel; reserve cumulative trace budget against actual geometry; synchronize retirement and quarantine unsafe clears |
| [launch_gate.cpp](../../src/cuda_runtime/launch_gate.cpp) | `FutureGeometryScope`, `runtime_launch`, `kernel_launch` | Carry ordinary driver/mapped-runtime geometry only; refuse cooperative, graph, extras and opaque future launches while preserving synchronous forwarding |
| [hbf_device.cu](../../src/cuda_runtime/device/hbf_device.cu) | `__hbfsim_timing_future_issue_v1`, `__hbfsim_timing_future_poll_v1`, `__hbfsim_timing_future_wait_v1` | Optional FAST scalar TIMING-read validation/group reservation/poll/consume helpers and bounded module-owned trace; GPU gold remains separate |
| [coverage.cpp](../../src/cuda_runtime/coverage.cpp) | `future_contract_json`, `future_contract_identity`, `future_kernel_contract_symbol`, `CoverageGate::future_module_contract` | Domain-separated original/mode/helper/layout/resource identity and exact per-kernel manifest binding; verify actual grid/block bounds before admission |
| [plugin.cpp](../../src/ptxpass_hbf/plugin.cpp), [transform.cpp](../../src/ptxpass_hbf/transform.cpp) | `TrustedModuleRegistry::identity_for`, `transform_ptx` | Complete opt-in future dispatch, original-byte recomposition of same-mode kernels and exact helper/contract identity; reject mixed mode or forged symbols |
| [hf_routing_worker.py](../../adapters/vllm_capacity/hf_routing_worker.py) | `fixed_prompt_members`, `make_protocol`, `make_capture_set_protocol`, `make_trace_composition`, `llm_arguments`, `make_environment`, `serialize_return` | Derive strict single-member or fixed 16-member token/route controls, bind two seed-0 trace-composed B=8 indexes, prepare private caches and fixed single-GPU/no-addon startup controls, and validate copied return arrays; composition is not live scheduling or capture authentication |
| [hf_runtime_contract.py](../../adapters/vllm_capacity/hf_runtime_contract.py) | `observe_runtime` | Inspect actual engine/worker config identities, raw execution model, per-layer attention/MoE/native sampler bindings and KV layout including the reserved null block; detached configuration observation only, no capture origin |
| [hf_owned_routes.py](../../adapters/vllm_capacity/hf_owned_routes.py) | `OwnedRouteMemory._open`, `reconcile`, `finish` | Restrict the owned capturer module to one exclusive buffer and one matching reader; retain pending original-class handles, check descriptor/namespace identity, clean independently and restore the local binding; no atomic protection against hostile namespace replacement |
| [hf_runtime_sources.py](../../scripts/eval/hf_runtime_sources.py) | `collect_runtime_sources`, `validate_runtime_sources`, `recheck_runtime_sources` | Bound selected installed Python/distribution inputs and interpreter identity; reconcile frozen buffers without source reads, then separately recheck originals; no RECORD target traversal or complete binary/capture authentication |
| [ptx_ir.cpp](../../src/ptxpass_hbf/ptx_ir.cpp) | `parse_module_spanned` | Preserve exact entry/header/statement spans and real scalar/setup def-use; reject unsupported executable strings, malformed headers/operands and unproved source control flow |
| [future_transform.cpp](../../src/ptxpass_hbf/future_transform.cpp) | `emit_timing_futures` | Finite producer slots, executed-path issue/native-load/wait-return dependencies and drains; caller enforces actual geometry against the declared block limit |
| [hbf_device.cu](../../src/cuda_runtime/device/hbf_device.cu) | `__hbfsim_timing_future_native_store_guard_v1` | Validate entire native-store span against every HBF range before write, including overlap/overflow; no media reservation |
| [hf_runtime_imports.py](../../scripts/eval/hf_runtime_imports.py) | `observe_runtime_imports` | Inspect retained loaded module origins, exact cache/log/temp paths and finite settings; avoid manager/import/cache APIs, reject live manager environment, binding replacement and any optional DLPack `_LIB` field under the declared opt-out; passive configuration evidence with injected inputs always MOCK |
| [hf_loaded_arm.py](../../scripts/eval/hf_loaded_arm.py) | `run_loaded_arm` | Private composition of an already-prepared owned request; the default path remains one generate, while the explicit routing-capture cell reuses one loaded model for 16 sequential single-sequence generates and saves each raw return/route/summary before composing two indexes; retained ownership/cleanup and parent/controlled-import/gold gates remain separate |
| [hf_moe_tuning.py](../../scripts/eval/hf_moe_tuning.py) | `collect_tuning_inputs`, `validate_tuning_inputs`, `recheck_tuning_inputs` | Bind one metadata-derived packaged tuning JSON or absence to a declared device name and frozen source ancestors; reject redirection before read; actual device/weights/override/cache selection remains a worker gate |
| [hf_moe_tuning_runtime.py](../../scripts/eval/hf_moe_tuning_runtime.py) | `retain_tuning_runtime`, `observe_loaded_tuning` | Retain preconstruction module/function/quant aliases; observe exact loaded MoE/Parameter geometry and baseline batch state; recheck raw stored state and report final environment wrapper; passive bounded evidence only, no inference import or scientific receipt |
| [run_matrix.py](../../scripts/eval/run_matrix.py) | `run_child(..., bootstrap_no_site=False)` | Optional strict-boolean `-S -B` for the existing ownership wrapper; preserve default argv and boot/PID/start acknowledgement, resource checks and owned cleanup; target imports/environment remain the caller's responsibility |
| [hf_route_array.py](../../scripts/eval/hf_route_array.py) | `decode_route_array` | Decode only bounded NPY v1/v2 integer C-order buffers after exact protocol/header/length checks; enforce expert range and top-k uniqueness, return immutable tuples; no NumPy import or capture-origin claim |
| [hf_startup_sources.py](../../scripts/eval/hf_startup_sources.py) | `collect_startup_sources`, `validate_startup_sources`, `recheck_startup_sources` | Bind seven fixed startup Python/stub files to the primary MOE_TUNING_V1 source identity, shared ancestors and combined budget; validate immutable buffers separately from descriptor-anchored current-file reads; no binary or scientific authentication |
| [hf_owned_worker.py](../../scripts/eval/hf_owned_worker.py) | `capture_project_sources`, `recheck_project_sources`, `_read_wire_retained` | Bind finite project sources and bounded wire bytes with retained namespace/file identities; no generic discovery or full binary authentication |
| [hf_owned_worker.py](../../scripts/eval/hf_owned_worker.py) | `execute_owned`, `_request`, `_loaded_plan`, `_torch_device_gate`, `_vllm_device_gate` | Controlled child startup, exact default or named fixed multi-prompt request/environment/ownership bindings and staged device/runtime gates before the loaded arm; independent postchecks and wrapper diagnostics; CPU/MOCK controls do not authenticate an actual GPU run |
| [hf_routing_runner.py](../../scripts/eval/hf_routing_runner.py) | `run_triplet`, `_routing_capture_request`, `_routing_capture_prompts`, `_ownership_callback`, `_validate_owned_bootstrap` | Parent CPU orchestration of the default three fresh sequential arms or the explicit single capture arm for `routing_capture-01469`; bind fixed prompts/member artifacts/two seed-0 indexes, immutable acknowledgement/bootstrap and owned cleanup; all returned status remains provisional and unvalidated |
| [hf_routing_runner.py](../../scripts/eval/hf_routing_runner.py) | `_read_owned_status_binding`, `_signal_number` | Bind exclusively created status and attempt identities, account for signals through the post-fsync cutoff and preserve earlier diagnostics during an owned status update |
| [hf_frozen_trace_verifier.py](../../scripts/eval/hf_frozen_trace_verifier.py) | `FrozenTraceArm`, `validate_frozen_trace` | Independently reconcile bounded immutable metadata, original protocol bytes, raw tokens/routes, tensor events/pages and summaries across three supplied arms; ROUTE_CONSISTENCY_ONLY, without process-origin or scientific authentication |
| [hf_route_cuda_events.py](../../adapters/vllm_capacity/hf_route_cuda_events.py) | `RouteCudaEvents.start`, `_capture`, `_save`, `_read`, `finish`, `close`, `snapshot` | Instance-owned reversible CUDA Event binding: preinitialize before measurement, join actual capture/save/read indices, and emit route-to-route intervals with an explicit missing terminal; includes scheduling/capture overhead and is not kernel-only compute |
| [hf_route_horizon_inputs.py](../../scripts/eval/hf_route_horizon_inputs.py) | `validate_capture`, `capacity_contract`, `prepare_route_horizon` | Decode bounded frozen route/event inputs without NumPy, bind 336 demand nodes/335 gaps, recompute requested capacity separately from achieved whole-expert rho, and preserve terminal censoring; no capture-origin or physical-residency authentication |
| [c6_future_lifecycle_candidate.ptx](../../benchmarks/cuda/c6_future_lifecycle_candidate.ptx) | `c6_future_lifecycle_candidate` | One 32-lane ordinary-future producer with executed overwrite, false overwrite/consume and unused-exit predicates; this distinct module has its own exact optimized cubin and retained mapping |
| [c6_future_lifecycle_correctness.cu](../../benchmarks/cuda/c6_future_lifecycle_correctness.cu) | `main`, `check_case`, `AcquisitionJournal`, `cleanup_resources` | Run three fixed cases with per-case sentinels, raw counters/traces saved before assertions, exact retained-image load and checked module/range retirement; void context-destroy completion remains unobservable |
| [run_c6_future_lifecycle_correctness.py](../../scripts/eval/run_c6_future_lifecycle_correctness.py) | `require_frozen_identities`, `expected_lane_output`, `validate_lifecycle_observations`, `execute`, `finalize_attempt` | Refuse unfilled identities, independently reconcile 96 outputs/32 consumes/64 drains/192 traces and invoke one guarded child; preserve failures/signals and CAPTURED_UNVALIDATED without full lifecycle, D/W or G5 promotion |
| [run_known_delay_per_chain_abba.py](../../scripts/eval/run_known_delay_per_chain_abba.py) | `analyze_abba`, `execute`, `finalize_attempt` | Require actual single-block/row/eight-event geometry, retain fixed-runtime A/B/B/A launches and independently compute signed adjacent wait/chain/Event deltas; two descriptive pairs do not close matrix-wide G2 |
