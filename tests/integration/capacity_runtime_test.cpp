...
int main()
{
    ...
    fakeCudaCapacityReset();
    {
        ...
        const auto stats = runtime->stats_v2();
        CHECK(stats.demand_requests == 2);
        CHECK(stats.cache_hits == 0);
        CHECK(stats.cache_misses == 2);
        CHECK(stats.hbf_read_bytes == 2 * kPageBytes);
        CHECK(stats.hbf_program_bytes == kPageBytes);
        CHECK(stats.writeback_bytes == kPageBytes);
        CHECK(stats.clean_evictions == 0);
        CHECK(stats.dirty_evictions == 1);
        CHECK(stats.completed_residences == 1);
        CHECK(stats.resident_pages_current == 1);
        CHECK(stats.resident_pages_peak == 2);
        CHECK(stats.free_frames == 1);
        CHECK(stats.frame_count == 2);
