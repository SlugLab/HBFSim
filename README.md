...
into that module. The helper validates control ABI v5 and the exact control
...
## Optional package-coupled thermal subsystem

Package thermal support is additive and defaults to off. A normal build has no
3D-ICE, NVML, ROM, or thermal-plugin requirement:

```bash
cmake -S . -B build-off \
  -DHBFSIM_ENABLE_CUDA=OFF \
  -DHBFSIM_ENABLE_MQSIM=ON \
  -DHBFSIM_ENABLE_PACKAGE_THERMAL=OFF \
  -DBUILD_TESTING=ON
cmake --build build-off -j
ctest --test-dir build-off --output-on-failure
```

Build the CPU/MQSim package path in a separate tree:
...
Existing callers keep using `hbfsim_context_create`; it is exactly equivalent
to package thermal off. Opt-in callers use the additive v2 options:
...
hbfsim_options_v2 options = {
    ...
    .package_thermal_stage = HBFSIM_PACKAGE_THERMAL_READ_ONLY,
    .package_thermal_model_kind = HBFSIM_PACKAGE_THERMAL_MODEL_ROM,
    .package_thermal_profile_path = "package-profile.json",
    .package_thermal_model_path = "package-rom.json",
};
hbfsim_context* context = NULL;
int status = hbfsim_context_create_v2(&options, &context);
```

Use `HBFSIM_PACKAGE_THERMAL_SHADOW` or `HBFSIM_PACKAGE_THERMAL_ACTIVE`
for later stages; the profile's `stage` must match the selected stage.
`read_only` reports temperature only, `shadow` reports would-have policy with
no service effect, and `active` applies light service scaling, severe admission
backpressure, and terminal shutdown fault behavior. Missing or invalid selected
inputs fail closed. When mode is off, package fields are deliberately ignored
and their paths are not inspected.

The checked-in 8Hi and 16Hi profiles and ROM are synthetic plumbing fixtures,
not calibrated package models. Run the complete synthetic offline validation:

```bash
ctest --test-dir build-package -R package_thermal_offline --output-on-failure
```

The external 3D-ICE 4.0 workflow, fitting/held-out validation, linear thermal
contribution decomposition, provenance rules, and exact commands are documented
in `plugins/package_thermal/offline/README.md`. Generate an experiment manifest
with `scripts/package_thermal_manifest.py --help`; it records commits, dirty
state, MQSim patch hashes, input hashes, thermal selection, and accelerator
environment. Runtime results are written to `package-thermal.json` in the
...
The completed controlled Phase-II workflow lives in
`experiments/package_thermal/phase2/`; its scientific reports are under
`reports/phase2/`.  It uses corrected G1 8Hi/16Hi geometry, an exact 3D-ICE
4.0 reference at 10 ms, a held-out-validated 224-state ROM, timeline-based
closed-loop analysis, three-seed sustainable-load sweeps, coupling/resistance
diagnostics, a 0.25/0.5/0.75 Light-service-scale sweep, and one-factor
uncertainty analysis.

The coupling diagnosis includes a fixed-HBF-load GPU 30→300→30 W step and
HBM-Power-informed idle/read/derived-write arms.  Their millidegree, slightly
wrong-sign HBF response is retained as a ROM/model residual and supports a G1
cross-talk NO-GO rather than a positive coupling claim.

A seven-point literature sanity sweep also applies the published HBF aggregate
power/bandwidth curve to G1.  The published 53.72 W threshold point reaches
40.987°C in G1 rather than 80°C, so the paper temperature curve is not treated
as reproduced or calibrated: its package geometry, materials and cooling inputs
are not public enough to reconstruct independently.  The disclosed curve only
supports a 0.26–0.27 nJ/byte aggregate dynamic range, not NAND command energies.

The formal controlled results are 87.46 MB/s for 8Hi and 75.11 MB/s for 16Hi
against a 200 MB/s experiment injection capacity.  These are Class-C
sensitivity results, not hardware peak-bandwidth claims.  The HBF-dominant
closed-loop timeline is a GO; cross-talk as the primary G1 limiter is a NO-GO.
Only 10 ms passed the strict transient convergence gate—50 ms is not an
accepted runtime setting.

`run_app0_phase2.py` executes the CUDA baseline/off/read-only/shadow/active
stage matrix with artifact hashes.  The pinned llama.cpp/TinyLlama APP-1 now
also completes that five-stage v2 integration matrix with exact generated text,
but its explicit-delay probes do not establish application HBF traffic or
