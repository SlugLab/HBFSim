...
class ImportedCacheTests(unittest.TestCase):
    def setUp(self):
        ...
        put('tvm_ffi._optional_torch_c_dlpack',_LIB=None)
    ...
    def test_complete_observation_is_detached_mock_and_never_calls_side_effect_apis(self):
        for module in self.modules.values():
            for name in ('gettempdir','get_dir','is_usage_stats_enabled','load_torch_c_dlpack_extension','get_artifacts_status','get_cache_manager','cache_dir','download_file'):
    ...
    def test_dlpack_library_none_or_exact_private_cuda_path_only(self):
        module=self.modules['tvm_ffi._optional_torch_c_dlpack'];self.observe()
        library=ctypes.CDLL.__new__(ctypes.CDLL);module._LIB=library
        library._name=str(self.work/'cache/tvm-ffi/libtorch_c_dlpack_addon_torch29-cuda.so');self.observe()
        for name in ('/outside/libtorch_c_dlpack_addon_torch29-cuda.so',str(self.work/'cache/tvm-ffi/libtorch_c_dlpack_addon_torch29-cpu.so')):
            library._name=name
            with self.subTest(name=name),self.assertRaises(ValueError):self.observe()
    ...
    def test_optional_shadow_and_forbidden_import_reject(self):
        ...
        self.modules['torch_c_dlpack_ext.submodule']=ModuleType('torch_c_dlpack_ext.submodule')
        with self.assertRaises(ValueError):self.observe()
