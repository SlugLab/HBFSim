...
void ShadowFutureMachine::refresh_record(Record& record)
{
    ...
    const bool has_arrival = (record.future.flags & FutureTiming) != 0 ||
                             record.completion.has_value();
    if (has_arrival && record.future.ready_ns <= now_ns_ &&
        record.future.ready_ns <= record.deadline_ns) {
        const auto status = record.completion.has_value()
                                ? record.completion->status
                                : RequestStatus::Ready;
        const auto state = status != RequestStatus::Ready
                               ? FutureState::TerminalError
                               : (record.completion.has_value() &&
                                  record.completion->deferred_materialization
                                      ? FutureState::DeferredMaterialization
                                      : FutureState::Ready);
        make_terminal(record, state, status);
        return;
    }
...
std::uint64_t ShadowFutureMachine::next_progress_time() const
{
    ...
    for (const auto& [ticket, record] : records_) {
        ...
        if (((record.future.flags & FutureTiming) != 0 || record.completion) &&
...
ShadowFuture ShadowFutureMachine::issue(const ShadowFutureRequest& request)
{
    ...
    if (request.bytes == 0 ||
        (request.flags != FutureNative && request.flags != FutureTiming &&
         request.flags != FutureCapacity) ||
        (request.flags != FutureNative &&
         request.deadline_ns <= request.issue_ns)) {
        throw std::invalid_argument("invalid or unbounded shadow future request");
    }
    if (next_ticket_ == std::numeric_limits<std::uint64_t>::max()) {
        throw std::overflow_error("shadow future ticket exhaustion");
    }
    ...
    std::uint64_t ticket, const ShadowFutureCompletion& completion)
{
    ...
    refresh_record(record);
    if (record.future.state != FutureState::Issued || record.completion ||
        (record.future.flags & FutureTiming) != 0 ||
        completion.status == RequestStatus::Pending) {
        return false;
    }
    record.future.resolved_address = completion.resolved_address;
    record.future.ready_ns = std::max(now_ns_, completion.ready_ns);
