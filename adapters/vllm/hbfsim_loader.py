...
HBFSIM_VLLM_OPTIONS_V2_VERSION = 1
...
class TimingConfig:
    ...
    package_thermal_stage: str = "off"
    package_thermal_profile_path: str = ""
    package_thermal_model_path: str = ""
    ...
    def from_mapping(cls, raw: Mapping[str, Any]) -> "TimingConfig":
        ...
        thermal_stage = str(raw.get("package_thermal_stage", "off"))
        thermal_profile = str(raw.get("package_thermal_profile_path", ""))
        thermal_model = str(raw.get("package_thermal_model_path", ""))
        ...
        if thermal_stage not in _PACKAGE_THERMAL_STAGES:
            raise ValueError(
                "package_thermal_stage must be off, read_only, shadow, or active"
            )
        package_enabled = thermal_stage != "off"
        if package_enabled != bool(thermal_profile and thermal_model):
            raise ValueError(
                "package thermal profile/model are required together for a "
                "non-off stage"
            )
        return cls(
            ...
            package_thermal_stage=thermal_stage,
            package_thermal_profile_path=thermal_profile,
            package_thermal_model_path=thermal_model,
        )
...
class _NativeOptionsV2(ctypes.Structure):
    _fields_ = [
        ("struct_size", ctypes.c_uint32),
        ("version", ctypes.c_uint32),
        ("base", _NativeOptions),
        ("package_thermal_stage", ctypes.c_uint32),
        ("package_thermal_profile_path", ctypes.c_char_p),
        ("package_thermal_model_path", ctypes.c_char_p),
    ]
...
class NativeTimingSession:
    ...
    def __init__(self, config: TimingConfig):
        ...
        if config.package_thermal_stage == "off":
            status = self._library.hbfsim_vllm_session_create(
                ctypes.byref(options), ctypes.byref(self._handle)
            )
        else:
            extended = _NativeOptionsV2(
                ctypes.sizeof(_NativeOptionsV2),
                HBFSIM_VLLM_OPTIONS_V2_VERSION,
                options,
                _PACKAGE_THERMAL_STAGES[config.package_thermal_stage],
                config.package_thermal_profile_path.encode(),
                config.package_thermal_model_path.encode(),
            )
            status = self._library.hbfsim_vllm_session_create_v2(
                ctypes.byref(extended), ctypes.byref(self._handle)
            )
    ...
    def _bind(self) -> None:
        ...
        self._library.hbfsim_vllm_session_create_v2.argtypes = [
            ctypes.POINTER(_NativeOptionsV2),
            ctypes.POINTER(ctypes.c_void_p),
        ]
        self._library.hbfsim_vllm_session_create_v2.restype = ctypes.c_int
