...
constexpr std::array<Case, 4> kCases{{
    {"same_dense", 4, 0xffffffffU},
    {"distinct_dense", kPageBytes, 0xffffffffU},
    {"same_sparse", 4, kSparseMask},
    {"distinct_sparse", kPageBytes, kSparseMask},
}};
...
Options parse_options(int argc, char** argv) {
    ...
    require(profile.at("page_bytes").get<std::uint32_t>() == kPageBytes,
            "four-case diagnostic requires exact 4096-byte profile pages");
...
json check_case(const Case& item, const std::vector<std::uint32_t>& input_host,
    ...
    const auto active_lanes = popcount(item.active);
    const auto expected_groups = item.stride == 4 ? 1U : active_lanes;
    std::array<std::uint32_t, 32> expected{};
    std::array<std::uint32_t, 32> sentinels{};
    for (std::uint32_t lane = 0; lane < 32; ++lane) {
        const bool active = (item.active & (1U << lane)) != 0;
        const auto word_index = std::size_t{lane} * item.stride / 4;
        expected[lane] = independent_work(lane) +
                         (active ? input_host.at(word_index) : 0U);
    ...
    std::uint32_t mask = item.active;
    void* arguments[]{&input, &output, &seed, &stride, &mask};
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::fprintf(stderr, "c6_future_correctness_candidate: %s\n", error.what());
