...
inline constexpr std::uint32_t kThermalScaleOnePpm = 1'000'000;
inline constexpr std::uint32_t kThermalFlagTerminalFault = 1U << 0;

enum class PackageThermalMode : std::uint32_t {
    Off = 0,
    PackageRc = 1,
};

enum class PackageThermalStage : std::uint32_t {
    Off = 0,
    ReadOnly = 1,
    Shadow = 2,
    Active = 3,
...
enum class PackageThermalPolicyState : std::uint32_t {
    Normal = 0,
    Light = 1,
    Severe = 2,
    Shutdown = 3,
...
struct alignas(64) SharedControlHeader {
    ...
    std::uint32_t thermal_stage;
    std::uint32_t thermal_policy_state;
    std::uint32_t thermal_scale_ppm;
    alignas(8) std::uint64_t thermal_generation;
    alignas(8) std::uint64_t thermal_blocked_requests;
    std::uint32_t thermal_admission_open;
    std::uint32_t thermal_flags;
};

inline bool package_thermal_admission_open(
    const SharedControlHeader& header) noexcept
{
    const auto load = [](const std::uint32_t& value) noexcept {
        return std::atomic_ref<std::uint32_t>(
                   *const_cast<std::uint32_t*>(&value))
            .load(std::memory_order_acquire);
    };
    const auto mode = load(header.thermal_mode);
    const auto stage = load(header.thermal_stage);
    return mode != static_cast<std::uint32_t>(
                       PackageThermalMode::PackageRc) ||
           stage != static_cast<std::uint32_t>(
                        PackageThermalStage::Active) ||
           load(header.thermal_admission_open) != 0;
}
...
static_assert(offsetof(SharedControlHeader, thermal_mode) == 344);
static_assert(offsetof(SharedControlHeader, thermal_stage) == 348);
static_assert(offsetof(SharedControlHeader, thermal_policy_state) == 352);
static_assert(offsetof(SharedControlHeader, thermal_scale_ppm) == 356);
static_assert(offsetof(SharedControlHeader, thermal_generation) == 360);
static_assert(offsetof(SharedControlHeader, thermal_blocked_requests) == 368);
static_assert(offsetof(SharedControlHeader, thermal_admission_open) == 376);
static_assert(offsetof(SharedControlHeader, thermal_flags) == 380);
...
inline std::uint64_t atomic_fetch_and(
    std::uint64_t& target, std::uint64_t bits,
    std::memory_order order = std::memory_order_relaxed) noexcept
{
    return std::atomic_ref<std::uint64_t>(target).fetch_and(bits, order);
}
...
public:
    ...
    bool initialize(std::uint32_t capacity) noexcept
    {
        ...
        h->thermal_scale_ppm = kThermalScaleOnePpm;
        ...
        RequestAdmissionHook after_reservation, void* hook_state) noexcept
    {
        ...
        if ((admission & kAdmissionClosedBit) != 0 ||
            !package_thermal_admission_open(*h)) {
            return false;
        }
