...
This candidate captures three predicated-first-consumer cases. A successful capture
...
REVIEWED_CUBIN_BYTES = 79760
REVIEWED_CUBIN_SHA256 = "0804821c83079e92f1a25610cb345c4e98ff2618bbb64d578b084537338e134f"
ORIGINAL_PTX_SHA256 = "51db6dd9a598d266210a18d45abdea461f3627e851451eca4986476552504523"
REVIEWED_TRANSFORMED_PTX_BYTES = 150174
REVIEWED_TRANSFORMED_PTX_SHA256 = "e4eac2951a2d9b2e5d4c5c80a5924824881bf6fba67425127e08de9ef8f0ced3"
REVIEWED_BUILD_MANIFEST_SHA256 = "aec0b3547969863bd90ef417183c259846a05beae203497977440726a48141da"
REVIEWED_DISASSEMBLY_MANIFEST_SHA256 = "3f59dbacd12e49b791d397cd8000db78ca1f833fcdd2517a95d4ecae1d3ba773"
REVIEWED_NVDISASM_SHA256 = "bfd9bd9d35eba8ff72fa4b355b65b2658d0584aae5cc7e5822866957e8094503"
REVIEWED_CUOBJDUMP_SHA256 = "eff3baec13cef3750774724ce9e877f597de31cdc62165102d96f1c9ff2e38f4"
SELECTED_KERNEL = "c6_conditional_consumer_candidate"
CONDITIONAL_CASES = (("first_false", 0), ("first_true", 0xffffffff),
                     ("first_mixed", 0x01010101))
...
def require_frozen_identities() -> None:
    ...
    if (REVIEWED_CUBIN_BYTES <= 0 or REVIEWED_TRANSFORMED_PTX_BYTES <= 0 or
            any(len(value) != 64 or any(char not in "0123456789abcdef" for char in value)
                for value in hashes)):
        raise ValueError("conditional-consumer native image identities are UNFROZEN")


def expected_lane_output(lane: int, first_consumer_mask: int) -> int:
    mask = 0xffffffff
    ...
    first = ((value + work) & mask) if first_consumer_mask & (1 << lane) else work
    return value ^ first


def validate_conditional_observations(raw: dict) -> None:
    """Check this fixed three-case output and once-per-producer acquisition."""
    ...
    require(raw.get("kernel") == SELECTED_KERNEL and raw.get("seed") == 0x9e3779b9 and
            ...
            raw.get("profile_page_bytes") == 4096, "conditional diagnostic configuration mismatch")
    ...
    cases = raw.get("cases", [])
    require(len(cases) == 3, "diagnostic did not retain the three conditional-consumer cases")
    ...
    input_base = None
    for case, (name, first_mask) in zip(cases, CONDITIONAL_CASES):
        require(case.get("case") == name and case.get("first_consumer_mask") == first_mask and
                case.get("first_consumer_lanes") == first_mask.bit_count() and
                ...
                "conditional case/mask/producer group mismatch")
        expected = [expected_lane_output(lane, first_mask) for lane in range(32)]
        require(case.get("observed_outputs") == expected, "conditional-consumer output mismatch")
        require(case.get("sentinel_echo") == case.get("sentinel_outputs") ==
                [value ^ 0xffffffff for value in expected], "conditional case sentinel mismatch")
        checksum = 0
        for value in expected:
            checksum = ((checksum * 131) ^ value) & 0xffffffffffffffff
        require(case.get("observed_checksum") == checksum, "conditional checksum mismatch")
        outputs = case.get("outputs", [])
        require(len(outputs) == 32 and all(
            item.get("lane") == lane and item.get("active") is True and
            item.get("first_consumer_executed") is bool(first_mask & (1 << lane)) and
            item.get("observed") == expected[lane] for lane, item in enumerate(outputs)),
            "conditional lane output records mismatch")
        before = dict(counts)
        ...
        require(case.get("counters_before") == before and case.get("counters_after") == counts and
                ...
                "conditional producer/counter conservation failed")
        traces = case.get("raw_traces", [])
        require(traces == case.get("traces") and len(traces) == 64,
                "conditional trace conservation failed")
        ...
        require(window.get("before_count") == before["trace_count"] and
                ...
                "conditional trace window invalid")
        ...
        for lane in range(32):
            ...
            require(len(pair) == 2 and [item.get("event") for item in pair] == [0, 5] and
                    [item.get("status") for item in pair] == [0, 1],
                    "producer did not issue and consume exactly once")
            issue, consumed = pair
            ...
            for item in pair:
                require(item.get("address") == input_base + 4 * lane and item.get("bytes") == 4 and
                        ...
                        "conditional trace identity/group mismatch")
            require(issue.get("issue_ns") == consumed.get("issue_ns") and
                    ...
                    "conditional trace timestamps are inconsistent")
            group_times.add((issue["issue_ns"], issue["ready_ns"]))
        require(len(group_times) == 1, "same-page lanes disagree on modeled readiness")
    require(raw.get("final_counters") == counts, "conditional final counters mismatch")
...
def execute(out: pathlib.Path, build: pathlib.Path, profile: pathlib.Path,
    ...
    paths = {
        "binary": build / "benchmarks/cuda/c6_conditional_consumer_correctness",
        "ptx": ptx,
        "cubin": cubin,
        ...
        "benchmark_source": ROOT / "benchmarks/cuda/c6_conditional_consumer_correctness.cu",
    ...
    try:
        ...
        with ResourceGuard(ROOT / "results", out, task) as guard:
            ...
            validate_conditional_observations(raw)
...
def main() -> int:
    ...
    plan = {
        "resource_class": "GPU_EXCLUSIVE",
        "cases": [name for name, _mask in CONDITIONAL_CASES],
        "first_consumer_masks": [mask for _name, mask in CONDITIONAL_CASES],
