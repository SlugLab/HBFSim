...
REQUIRED = ('python', 'runner', 'model_config', 'profile', 'torch_overlay',
            ...
            'expect_all98', 'expect_all98_plus_qkv')
STAGED = ('wrapper', 'bundle_manifest', 'agent', 'compiler', 'agent_build_receipt',
          ...
          'stage_receipt', 'provider', 'provider_cpu_receipt',
          'qkv_worker_plugin', 'qkv_worker_entry', 'qkv_worker_cpu_receipt',
          'qkv_abi_map', 'qkv_source_ptx', 'qkv_staged_ptx')
...
def verify_config(config_path, root, staged=True):
    ...
    for scope in ('layer0', 'all98', 'all98_plus_qkv'):
    ...
    original = load(files['expect_all98'])['selected']
    universe = load(files['expect_all98_plus_qkv'])['selected']
    check(len(original) == 98 and len(universe) == 99 and universe[:98] == original,
          'union expectation changed the original 98 storage records')
    check(universe[98]['aliases'] == ['model.layers.0.self_attn.qkv_proj.weight']
          and universe[98]['storage_bytes'] == 25165824,
          'union expectation did not add the one exact QKV weight')
    for scope in ('layer0', 'all98', 'all98_plus_qkv'):
    ...
    if staged:
        combined = cfg.get('combined_runtime', {})
        check(combined.get('scope') == 'all98_plus_qkv' and
              combined.get('worker_plugin_status') == 'PINNED_CPU_CHECKED_NOT_GPU_VALIDATED' and
              combined.get('worker_plugin_root') == str(files['qkv_worker_plugin'].parent.parent),
              'combined worker plugin identity incomplete')
        check('[vllm.general_plugins]' in files['qkv_worker_entry'].read_text() and
              'hbfsim_qkv_model_adapter = qkv_model_adapter:register' in
              files['qkv_worker_entry'].read_text(), 'QKV entry point missing')
        for key, role in (('HBFSIM_QKV_ABI_MAP_SHA256', 'qkv_abi_map'),
                          ('HBFSIM_QKV_STAGED_PTX_SHA256', 'qkv_staged_ptx'),
                          ('HBFSIM_QKV_SOURCE_PTX_PATH', 'qkv_source_ptx'),
                          ('HBFSIM_QKV_STAGED_PTX_PATH', 'qkv_staged_ptx')):
            expected = digest(files[role]) if key.endswith('SHA256') else str(files[role])
            check(combined.get(key) == expected, f'combined {key} differs from pinned artifact')
...
def environment(cfg, files, root, out, staged):
    norm_diagnostic = os.environ.get('HBFSIM_SCOPED_NORM_DIAGNOSTIC')
    dispatch_observe = os.environ.get('HBFSIM_NATIVE_DISPATCH_OBSERVE')
    if norm_diagnostic is not None and norm_diagnostic != '1':
        raise ValueError('HBFSIM_SCOPED_NORM_DIAGNOSTIC must be 1 when set')
    if dispatch_observe is not None and dispatch_observe != '1':
        raise ValueError('HBFSIM_NATIVE_DISPATCH_OBSERVE must be 1 when set')
    env = os.environ.copy()
    for key in list(env):
    ...
    cache = out / 'cache'
    env.update({
        'PYTHONPATH': ':'.join(map(str, ((files['qkv_worker_plugin'].parent.parent,
                                          files['runner'].parent)
                                         if staged and combined.get('scope') == 'all98_plus_qkv'
                                         else ()) + (vllm, torch, site))),
    ...
    if staged:
        ...
        if combined.get('scope') == 'all98_plus_qkv':
            check(combined['BPFTIME_CUDA_EXACT_BIND_ONLY'] == '0' and
                  combined['HBFSIM_QKV_RECOVERED_ABI_V1'] == '1' and
                  combined['target_only_provider_path'] == str(files['provider']),
                  'combined QKV runtime identity changed')
            env.update({
                'BPFTIME_CUDA_EXACT_BIND_ONLY': '0',
                'HBFSIM_QKV_RECOVERED_ABI_V1': '1',
                'HBFSIM_QKV_ABI_MAP_SHA256': combined['HBFSIM_QKV_ABI_MAP_SHA256'],
                'HBFSIM_QKV_SOURCE_PTX_PATH': combined['HBFSIM_QKV_SOURCE_PTX_PATH'],
                'HBFSIM_QKV_STAGED_PTX_PATH': combined['HBFSIM_QKV_STAGED_PTX_PATH'],
                'HBFSIM_QKV_STAGED_PTX_SHA256': combined['HBFSIM_QKV_STAGED_PTX_SHA256'],
                'HBFSIM_QKV_MODEL_PLUGIN_V1': '1',
                'HBFSIM_QKV_MODEL_RECEIPT_DIR': str(out / 'qkv-plugin'),
                'HBFSIM_QKV_ABI_DECISION_LOG': str(out / 'qkv-abi-decision.jsonl'),
                'HBFSIM_TARGET_EXTRA_LD_PRELOAD': str(files['provider']),
                'HBFSIM_PROVIDER_TRACE_LOG': str(out / 'provider-blas.jsonl'),
                'HBFSIM_PROVIDER_CORRELATION_LOG': str(out / 'provider-correlation.jsonl'),
                'HBFSIM_PROVIDER_MODULE_DIR': str(out / 'provider-modules'),
            })
            env.pop('VLLM_PLUGINS', None)
        if norm_diagnostic == '1':
            env['HBFSIM_SCOPED_NORM_DIAGNOSTIC'] = '1'
        if dispatch_observe == '1':
            env['HBFSIM_NATIVE_DISPATCH_OBSERVE'] = '1'
...
def target_maps(pid, seen):
    paths = {line.split()[-1] for line in Path(f'/proc/{pid}/maps').read_text().splitlines()
             if ('libtorch_cuda.so' in line or '_C.abi3.so' in line or '_moe_C.abi3.so' in line
                 or 'libhbfsim_launch_gate.so' in line or 'libbpftime-agent.so' in line
                 or 'libprovider_qkv_production.so' in line)}
    for name in paths:
...
def verify_qkv_selected_call(out):
    directory = out / 'qkv-plugin'
    logs = sorted(directory.glob('worker-*.jsonl'))
    check(len(logs) == 1, 'expected exactly one QKV model worker receipt')
    events = [json.loads(s) for s in logs[0].read_text().splitlines() if s.strip()]
    names = ('installed', 'scheduled_prefill', 'scheduled_decode',
             'selected_activation', 'selected_qkv')
    check([r.get('event') for r in events] == list(names),
          'QKV worker event sequence is incomplete or repeated')
    check(all(r.get('schema_version') == 1 and r.get('pid') == events[0]['pid']
              for r in events), 'QKV worker event identity changed')
    installed, prefill, decode, activation, selected = events
    check(installed.get('status') == 'READY' and
          installed.get('weight_bytes') == 25165824,
          'exact QKV weight was not installed')
    check(prefill.get('status') == 'OBSERVED' and prefill.get('positions') == [0, 1]
          and decode.get('status') == 'ARMED' and len(decode.get('positions', [])) == 1
          and decode['positions'][0] > 0, 'same-request prefill/decode missing')
    request_id = prefill.get('request_id')
    check(request_id and all(r.get('request_id') == request_id
                             for r in (decode, activation, selected)),
          'QKV worker request identity changed')
    check(prefill.get('num_reqs') == decode.get('num_reqs') == 1 and
          prefill.get('speculative_decode') is False and
          decode.get('speculative_decode') is False,
          'selected QKV was outside the one-request ordinary decode')
    check(activation.get('status') == 'CAPTURED' and
          activation.get('activation_bytes') == 4096 and
          activation.get('activation_shape') == [1, 2048] and
          activation.get('activation_dtype') == 'torch.bfloat16' and
          activation.get('activation_ptr') and
          int(activation.get('stream_handle', '-1'), 16) >= 0,
          'selected activation metadata incomplete')
    for key in ('activation_sha256', 'activation_ptr', 'activation_shape',
                'activation_strides', 'activation_dtype', 'activation_device',
                'stream_handle', 'thread_id'):
        check(selected.get(key) == activation.get(key),
              f'selected call changed activation metadata: {key}')
    check(selected.get('status') == 'BYTE_EQUAL' and selected.get('selected_count') == 1
          and selected.get('output_bytes') == 12288 and
          selected.get('weight_ptr') == installed.get('weight_ptr') and
          selected.get('native_output_ptr') != selected.get('candidate_output_ptr'),
          'selected QKV did not return one distinct byte-equal output')
    prefix = f"worker-{events[0]['pid']}"
    raw = {
        'activation': directory / f'{prefix}-activation.bin',
        'native': directory / f'{prefix}-native-qkv.bin',
        'candidate': directory / f'{prefix}-candidate-qkv.bin',
    }
    check(all(p.is_file() for p in raw.values()) and
          raw['activation'].stat().st_size == 4096 and
          raw['native'].stat().st_size == raw['candidate'].stat().st_size == 12288,
          'QKV worker raw buffers absent or wrong size')
    check(digest(raw['activation']) == activation['activation_sha256'] and
          digest(raw['native']) == selected['native_sha256'] ==
          digest(raw['candidate']) == selected['candidate_sha256'],
          'QKV worker raw output hashes differ')
    decisions_path = out / 'qkv-abi-decision.jsonl'
    check(decisions_path.is_file(), 'QKV exact ABI decision log absent')
    decisions = [json.loads(s) for s in decisions_path.read_text().splitlines() if s.strip()]
    chosen = [r for r in decisions if r.get('decision') == 'PATCHED_SELECTED']
    check(len(chosen) == 1 and any(r.get('decision') == 'NATIVE_UNSELECTED'
                                   for r in decisions),
          'QKV did not have one selected patched launch and a native reference')
    decision = chosen[0]
    check(decision.get('cuda_result') == 0 and
          decision.get('original_function') not in (None, '0x0', '(nil)') and
          decision.get('patched_function') not in (None, '0x0', '(nil)') and
          decision['original_function'] != decision['patched_function'] and
          decision.get('context') not in (None, '0x0', '(nil)') and
          int(decision.get('association_token', 0)) > 0 and
          len(decision.get('grid', [])) == len(decision.get('block', [])) == 3 and
          all(int(x) > 0 for x in decision['grid'] + decision['block']),
          'selected QKV launch lacks exact handle/context/geometry proof')
    return {'status': 'PASS_ONE_SCHEDULED_QKV_BYTE_EQUAL',
            'worker_pid': events[0]['pid'], 'request_id': request_id,
            'weight_ptr': selected['weight_ptr'],
            'activation_sha256': digest(raw['activation']),
            'native_qkv_sha256': digest(raw['native']),
            'candidate_qkv_sha256': digest(raw['candidate']),
            'decision': decision, 'worker_receipt_sha256': digest(logs[0]),
            'decision_log_sha256': digest(decisions_path)}
...
def run_once(cfg, files, root, config_path, scope, staged, out, epoch, overall, baseline):
    ...
    if staged and scope == 'all98_plus_qkv':
        (out / 'qkv-plugin').mkdir(mode=0o700)
...
def verify_result(out, scope, cfg, files, baseline, staged, write_report=False):
    ...
    provider = [r for r in maps if r['path'] == str(files['provider'])]
    check(len(provider) == 1 and provider[0]['sha256'] == digest(files['provider']),
          'loaded target-only provider identity mismatch')
    ...
    qkv_module = 'ptx:sha256:6db074711d19c3e31cbcc98c6e170f0b4330259f220c2d518ce8aee42b9c9eab'
    old_module_ids = {row['module_id'] for row in
                      (json.loads(line) for line in files['pass_manifest'].read_text().splitlines()
                       if line.strip()) if row['module_id'] != qkv_module}
    if scope == 'all98_plus_qkv':
        check(len(old_module_ids) == 7 and qkv_module in modules,
              'union old/QKV modules absent from accounting')
    by_storage = []
    for storage in registration['storages']:
        base, length = storage['address'], storage['bytes']
        is_qkv = (storage['aliases'] == ['model.layers.0.self_attn.qkv_proj.weight'])
        allowed = ({qkv_module} if is_qkv else old_module_ids)
        ...
        active = sorted({r['module_id'] for r in matches if r['module_id'] in allowed
                         and r['module_id'] in modules
    ...
    union_detail = None
    if scope == 'all98_plus_qkv':
        old_rows = [r for r in by_storage if r['aliases'] !=
                    ['model.layers.0.self_attn.qkv_proj.weight']]
        qkv_rows = [r for r in by_storage if r['aliases'] ==
                    ['model.layers.0.self_attn.qkv_proj.weight']]
        qkv_counters = modules[qkv_module]['counters']
        qkv_closed = (modules[qkv_module]['status'] == 'COMPLETE'
                      and qkv_counters['modeled_admitted_accesses'] > 0
                      and len({qkv_counters[k] for k in names}) == 1
                      and len({qkv_counters[k] for k in byte_names}) == 1)
        qkv_call = verify_qkv_selected_call(out)
        check(len(qkv_rows) == 1 and
              int(qkv_call['weight_ptr'], 16) == qkv_rows[0]['base'],
              'selected QKV pointer differs from the registered exact weight storage')
        union_detail = {
            'old98_count': len(old_rows),
            'old98_bytes': sum(r['bytes'] for r in old_rows),
            'old98_all_addressed_active': len(old_rows) == 98 and
                                          all(r['active_complete_modules'] for r in old_rows),
            'old98_positive_modeled_modules': sum(
                modules[name]['counters']['modeled_admitted_accesses'] > 0
                for name in old_module_ids if name in modules),
            'qkv_count': len(qkv_rows),
            'qkv_exact_module': qkv_module,
            'qkv_module_closed': qkv_closed,
            'qkv_addressed_active': len(qkv_rows) == 1 and
                                    qkv_rows[0]['active_complete_modules'] == [qkv_module],
            'qkv_plugin_receipt': qkv_call,
        }
        check(union_detail['old98_bytes'] == 13091213312 and
              union_detail['old98_all_addressed_active'] and
              union_detail['qkv_addressed_active'] and qkv_closed,
              'old98 or selected QKV coverage/closure is incomplete')
    ...
    if union_detail is not None:
        report['union_detail'] = union_detail
...
def main():
    ...
    parser.add_argument('--scope', choices=('layer0', 'all98', 'all98_plus_qkv'), default='layer0')
