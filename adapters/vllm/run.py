...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--package-thermal-stage",
        choices=("off", "read_only", "shadow", "active"),
        default="off",
    )
    parser.add_argument("--package-thermal-profile")
    parser.add_argument("--package-thermal-model")
    parser.add_argument(
        "--hbf-timing-model",
...
def base_manifest(args: argparse.Namespace, cache: pathlib.Path) -> dict[str, Any]:
    return {
        ...
        "package_thermal": {
            "stage": args.package_thermal_stage,
            "profile": args.package_thermal_profile,
            "model": args.package_thermal_model,
        },
    }
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    package_enabled = args.package_thermal_stage != "off"
    if package_enabled != bool(
        args.package_thermal_profile and args.package_thermal_model
    ):
        raise SystemExit(
    ...
    with suppress_static_fatbin_scan(enabled=args.mode == "timing"):
        if args.mode == "timing":
            ...
            loader_extra = {
                ...
                "package_thermal_stage": args.package_thermal_stage,
                "package_thermal_profile_path": (
                    str(pathlib.Path(args.package_thermal_profile).resolve())
                    if args.package_thermal_profile else ""
                ),
                "package_thermal_model_path": (
                    str(pathlib.Path(args.package_thermal_model).resolve())
                    if args.package_thermal_model else ""
                ),
            }
