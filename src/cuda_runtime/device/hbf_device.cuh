...
HBFSIM_HOST_DEVICE constexpr std::uint64_t thermal_scaled_service_ns(
    std::uint64_t service_ns, std::uint32_t scale_ppm) noexcept
{
    if (service_ns == 0 || scale_ppm == 0 ||
        scale_ppm > kThermalScaleOnePpm) {
        return scale_ppm > kThermalScaleOnePpm ? 0 : UINT64_MAX;
    }
    const auto quotient = service_ns / scale_ppm;
    const auto remainder = service_ns % scale_ppm;
    if (quotient > UINT64_MAX / kThermalScaleOnePpm) {
        return UINT64_MAX;
    }
    const auto whole = quotient * kThermalScaleOnePpm;
    const auto fractional =
        (remainder * kThermalScaleOnePpm + scale_ppm - 1) / scale_ppm;
    return fractional > UINT64_MAX - whole ? UINT64_MAX
                                           : whole + fractional;
}
