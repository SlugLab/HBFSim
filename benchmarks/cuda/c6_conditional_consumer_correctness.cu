...
constexpr char kKernel[] = "c6_conditional_consumer_candidate";
constexpr char kOriginalPtxSha256[] =
    "51db6dd9a598d266210a18d45abdea461f3627e851451eca4986476552504523";
constexpr std::size_t kReviewedTransformedPtxBytes = 150174;
constexpr char kReviewedTransformedPtxSha256[] =
    "e4eac2951a2d9b2e5d4c5c80a5924824881bf6fba67425127e08de9ef8f0ced3";
constexpr std::size_t kReviewedCubinBytes = 79760;
constexpr char kReviewedCubinSha256[] =
    "0804821c83079e92f1a25610cb345c4e98ff2618bbb64d578b084537338e134f";
constexpr char kReviewedBuildManifestSha256[] =
    "aec0b3547969863bd90ef417183c259846a05beae203497977440726a48141da";
constexpr char kReviewedDisassemblyManifestSha256[] =
    "3f59dbacd12e49b791d397cd8000db78ca1f833fcdd2517a95d4ecae1d3ba773";
constexpr char kReviewedNvdisasmSha256[] =
    "bfd9bd9d35eba8ff72fa4b355b65b2658d0584aae5cc7e5822866957e8094503";
constexpr char kReviewedCuobjdumpSha256[] =
    "eff3baec13cef3750774724ce9e877f597de31cdc62165102d96f1c9ff2e38f4";
...
constexpr std::uint32_t kSparseMask = 0x01010101U;
...
struct Case {
    const char* name;
    std::uint32_t stride;
    std::uint32_t first_consumer_mask;
};

constexpr std::array<Case, 3> kCases{{
    {"first_false", 4, 0U},
    {"first_true", 4, 0xffffffffU},
    {"first_mixed", 4, kSparseMask},
}};
...
Options parse_options(int argc, char** argv) {
    ...
    require(profile.at("page_bytes").get<std::uint32_t>() == kPageBytes,
            "conditional-consumer diagnostic requires exact 4096-byte profile pages");
...
json check_case(const Case& item, const std::vector<std::uint32_t>& input_host,
    ...
    require(item.stride == 4, "conditional-consumer cases require one same-page producer group");
    ...
    for (std::uint32_t lane = 0; lane < 32; ++lane) {
        const bool first_consumes = (item.first_consumer_mask & (1U << lane)) != 0;
        const auto word_index = std::size_t{lane} * item.stride / 4;
        const auto value = input_host.at(word_index);
        const auto work = independent_work(lane);
        const auto first_value = first_consumes ? value + work : work;
        expected[lane] = value ^ first_value;
        sentinels[lane] = expected[lane] ^ 0xffffffffU;
        require(sentinels[lane] != expected[lane],
                "output sentinel must differ from expected value");
    }
    ...
    json raw{
        ...
        {"first_consumer_mask", item.first_consumer_mask},
        {"first_consumer_lanes", popcount(item.first_consumer_mask)},
    ...
    std::uint32_t first_consumer_mask = item.first_consumer_mask;
    constexpr std::uint32_t mask = producer_mask;
    void* arguments[]{&input, &output, &seed, &stride, &first_consumer_mask};
    ...
    require(subtract(after.consumed, before.consumed, "consumed") ==
                active_lanes,
            "consumed count differs from active lanes");
    require(subtract(after.drained, before.drained, "drained") == 0 &&
                subtract(after.terminal_error, before.terminal_error,
                         "terminal_error") == 0,
            "successful dependency wait has a non-consume terminal");
    ...
    require(reported_count == 2U * active_lanes,
            "expected one issue and one combined ready/consume trace per lane");
    ...
    for (std::uint32_t lane = 0; lane < 32; ++lane) {
        ...
        output_json.push_back({{"lane", lane},
                               {"active", active},
                               {"first_consumer_executed", (item.first_consumer_mask & (1U << lane)) != 0},
        ...
        for (const auto& trace : by_lane[lane]) {
            if (trace.event == 0 && trace.status == future::kPending) {
                ...
                issue = &trace;
                           ...
                           (future::kBecameReady | future::kConsumed) &&
        ...
        require(issue && terminal, "missing issue or ready/consume trace");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        require(kReviewedCubinBytes > 0 && kReviewedTransformedPtxBytes > 0,
                "conditional-consumer native image identities are UNFROZEN");
        ...
        std::fprintf(stderr, "c6_conditional_consumer_correctness: %s\n", error.what());
