...
mkdir -p ../eval-artifacts/tmp-cpu
TMPDIR="$PWD/../eval-artifacts/tmp-cpu" ctest --test-dir ../eval-artifacts/build-cpu --output-on-failure
...
mkdir -p ../eval-artifacts/tmp-tools
TMPDIR="$PWD/../eval-artifacts/tmp-tools" ctest --test-dir ../eval-artifacts/build-tools --output-on-failure
```
...
Use distinct TMPDIRs or run complete suites sequentially. A simultaneous suite run had a tuned-context startup failure; isolated reruns passed without runtime changes.
