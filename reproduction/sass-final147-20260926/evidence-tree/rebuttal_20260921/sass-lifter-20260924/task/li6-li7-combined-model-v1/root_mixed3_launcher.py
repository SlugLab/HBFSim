#!/usr/bin/env python3
"""Root-only future mixed3 prepare/launch; this file does not start on import."""
import argparse
import fcntl
import hashlib
import json
...
TEMPLATE=TASK/"MIXED3_PLAN_TEMPLATE.json"
LIVE=TASK/"MIXED3_LIVE_PLAN.json"
REVIEW=TASK/"ROOT_REVIEWED_MIXED3.json"
...
def dependency(path):
    receipt=load(path)
    need(receipt.get("status")=="MODEL_CONNECTED_PASS_LM_HEAD" and
         isinstance(receipt.get("epoch"),int),"head model representative PASS is absent")
    return {"head_validation_path":str(Path(path).resolve(strict=True)),
            "head_validation_sha256":sha(path),"head_validation_epoch":receipt["epoch"]}
...
def artifacts(plan):
    need(plan["template_sha256"]==sha(TEMPLATE) and
         ...
         plan["scope_manifest_sha256"]==sha(TASK/"prepared-v2/scope-mixed3.json"),
...
def prepare(args):
    need(not LIVE.exists() and not RUN.exists(),"mixed3 live plan/run already exists")
    need(args.epoch > 0 and args.gpu_index >= 0,"positive epoch and nonnegative configured GPU index required")
    dep=dependency(args.head_validation)
    renewal=load(args.renewal_receipt);live=load(args.live_resource_receipt)
    plan=load(TEMPLATE)
    need(plan["scope"]=="mixed3" and
         (plan["worker_seconds"],plan["outer_seconds"],plan["collection_seconds"],
          plan["startup_cleanup_seconds"],plan["full_admission_seconds"])==
         (1800,2100,600,300,3000),"mixed3 template budget changed")
    ...
    plan.update({"configured_gpu_index":args.gpu_index,
                 ...
                 "head_model_dependency":"MODEL_CONNECTED_PASS_LM_HEAD_VERIFIED",**dep})
    join=load(TASK/"bundle-combined-v1/BUNDLE_JOIN.json")
    ...
    save_new(LIVE,plan)
    print(json.dumps({"status":"MIXED3_LIVE_PLAN_PREPARED_NO_GPU_START",
...
def launch():
    ...
    need(not RUN.exists(),"mixed3 run already exists")
    plan=load(LIVE);review=load(REVIEW)
    need(review.get("status")=="ROOT_REVIEWED_COMBINED_MIXED3_V1" and
    ...
    need(dependency(plan["head_validation_path"])["head_validation_sha256"]==
         plan["head_validation_sha256"],"head model pass dependency changed")
    ...
    save_new(RUN/"owner-start.json",{"schema":"hbfsim.combined_mixed3_owner.v1",
...
def main():
    ...
    with OWNER_LOCK.open("a+") as lock:
        ...
        if args.prepare:
            need(all(x is not None for x in (args.head_validation,args.renewal_receipt,
                 args.live_resource_receipt,args.gpu_index,args.epoch)),
                 "prepare requires head PASS, finite lease, configured GPU and epoch")
            prepare(args)
        else:launch()
