...
| HOST-MEMORY | OPEN AT LAST OBSERVATION | HF005 failed with MemoryError at vllm-device after303.869927600s; metadata-current postcheck also failed, eight others passed. MemAvailable2,028,416kB and commit headroom4,337,432kB at launch. No model constructed; owned cleanup confirmed. HF004 resource failure remains historical. | Preserve both attempts; investigate installed CPU pool/thread allocation and wait for a meaningful resource or scoped implementation change before retry. Do not change system settings or stop unrelated jobs. Continue small C6 diagnostics. |
...
`b136c68b8c1e3c619babc8f445f8d295079e86bd`; attempts004/005 subsequently failed under host memory pressure before model construction. K1 hardware controls still fail G2. Formal acceptance and COMPLETE remain
