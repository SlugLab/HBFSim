...
Stage run(const Opt&o,std::string temp,hbfsim_context*c,CUfunction f,void*logical,u64*dout,const std::vector<u64>&b,u64 bytes){
 ...
 for(size_t r=0;r<b.size();r++){u64 base=b[r],words=actual.size();std::uint32_t base_lo=static_cast<std::uint32_t>(base),base_hi=static_cast<std::uint32_t>(base>>32);void*args[]{&logical,&base_lo,&base_hi,&words,&dout};dok(cuLaunchKernel(f,1,1,1,1,1,1,0,nullptr,args,nullptr),"loads");cok(cudaDeviceSynchronize(),"sync");cok(cudaMemcpy(actual.data(),dout,bytes,cudaMemcpyDeviceToHost),"result");
...
int main(int ac,char**av){hbfsim_context*c=nullptr;void*logical=nullptr;CUmodule m=nullptr;u64*out=nullptr;std::filesystem::path path,pending;
 ...
 hbfsim_options co{.profile_path=o.profile.c_str(),.report_dir=o.report.c_str(),.mode=HBFSIM_MODEL_REFERENCE,.ring_capacity=64,.request_timeout_ns=30000000000ULL};if(hbfsim_context_create(&co,&c)!=HBFSIM_OK)die("context");
 ...
 using exact_bind_type=int(*)(CUfunction,const char*,size_t,const char*);
 auto exact_bind=reinterpret_cast<exact_bind_type>(dlsym(RTLD_DEFAULT,"bpftime_nv_bind_ptx_variant"));
 req(exact_bind!=nullptr,"exact PTX binder unavailable");
 req(exact_bind(f,ptx.data(),ptx.size(),"r3_page_read_kernel")==0,"exact PTX bind failed");
 fprintf(stderr,"HBFSIM_R3_EXACT_BIND PASS kernel=r3_page_read_kernel ptx_bytes=%zu\n",ptx.size());
