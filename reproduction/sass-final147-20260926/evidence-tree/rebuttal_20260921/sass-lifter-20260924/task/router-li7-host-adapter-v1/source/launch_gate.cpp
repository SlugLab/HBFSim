...
int first_fault_begin(std::uint64_t epoch, const char* path) noexcept
{
    try {
        ...
        const auto host_register_rc = ::cudaHostRegister(
            fresh.mapping, fresh.mapping_bytes,
            cudaHostRegisterMapped | cudaHostRegisterPortable);
        if (host_register_rc != cudaSuccess) {
            Dl_info runtime_dso{};
            const auto runtime_symbol = ::dlsym(RTLD_DEFAULT, "cudaHostRegister");
            const char* runtime_path = runtime_symbol != nullptr &&
                    ::dladdr(runtime_symbol, &runtime_dso) != 0
                ? runtime_dso.dli_fname : "UNRESOLVED";
            std::fprintf(stderr,
                "HBFSIM_FIRST_FAULT_INIT stage=cudaHostRegister rc=%d lookup_dso=%s mapped_bytes=%zu\n",
                static_cast<int>(host_register_rc), runtime_path,
                fresh.mapping_bytes);
            ::munmap(fresh.mapping, fresh.mapping_bytes);
            ::close(fresh.fd);
            return -5;
        }
        const auto device_pointer_rc = ::cudaHostGetDevicePointer(
            &fresh.device_mapping, fresh.mapping, 0);
        if (device_pointer_rc != cudaSuccess) {
            Dl_info runtime_dso{};
            const auto runtime_symbol = ::dlsym(RTLD_DEFAULT, "cudaHostGetDevicePointer");
            const char* runtime_path = runtime_symbol != nullptr &&
                    ::dladdr(runtime_symbol, &runtime_dso) != 0
                ? runtime_dso.dli_fname : "UNRESOLVED";
            std::fprintf(stderr,
                "HBFSIM_FIRST_FAULT_INIT stage=cudaHostGetDevicePointer rc=%d lookup_dso=%s mapped_bytes=%zu\n",
                static_cast<int>(device_pointer_rc), runtime_path,
                fresh.mapping_bytes);
                                             ...
                                             void** arguments, void** extra)
{
    ...
    if (extra != nullptr) {
        ...
        return require_timing_binding(
            inspect_bounded(canonical_function(function), module_id, kernel,
                            get_info, read_packed),
            function);
    }
                       ...
                       void** parameters, void** extra)
{
    ...
    // The original cuBLAS QKV entry has one 152-byte aggregate parameter,
    // while its recovered exact alias has 19 parameters. Normal inspection
    // canonicalizes to the alias and would read past the original pointer
    // array. Hand this single opt-in call to the agent before inspection; the
    // agent checks live image/context/ABI and calls this gate on 19 cells.
    const char* qkv_opt_in = std::getenv("HBFSIM_QKV_RECOVERED_ABI_V1");
    if (qkv_opt_in && std::strcmp(qkv_opt_in, "1") == 0) {
        ...
        bool exact_qkv = name == kQkvName || name == kLi7Name;
        if (!exact_qkv && name == "unknown_cufunction") {
            using exact_identity_type = int (*)(CUfunction);
            auto exact_identity = reinterpret_cast<exact_identity_type>(dlsym(
                RTLD_DEFAULT, "bpftime_nv_qkv_is_exact_original_v1"));
            exact_qkv = exact_identity && exact_identity(function) == 1;
        }
        if (exact_qkv) {
            if (std::strcmp(symbol, "cuLaunchKernel") != 0)
                return finish(CUDA_ERROR_NOT_SUPPORTED);
            using exact_type = CUresult (*)(
                CUfunction, unsigned int, unsigned int, unsigned int,
                unsigned int, unsigned int, unsigned int, unsigned int,
                CUstream, void**, void**);
            auto exact = reinterpret_cast<exact_type>(dlsym(
                RTLD_DEFAULT, "bpftime_nv_qkv_exact_aggregate_launch_v1"));
            if (!exact) return finish(CUDA_ERROR_NOT_SUPPORTED);
            return finish(exact(function, grid_x, grid_y, grid_z, block_x,
                                block_y, block_z, shared_memory, stream,
                                parameters, extra));
        }
    }
...
// Read-only exact registration check for the opt-in one-call QKV scope.
extern "C" int hbfsim_qkv_registered_weight_span_v1(
    std::uint64_t base, std::size_t bytes) noexcept
{
    if (!base || !bytes) return 0;
    try {
        for (const auto& span : runtime_gate().registered_snapshot())
            if (span.base == base && span.bytes == bytes) return 1;
    } catch (...) { return 0; }
    return 0;
}

// The agent supplies the converted 19 cells here. This is the ordinary gate
// launch lifetime: guard + geometry + one inspect + scoped approval, then the
// existing strict bridge forwards exactly one real Driver call. Nested gate
// approvals consume the scoped decision and do not inspect the cells again.
extern "C" CUresult hbfsim_qkv_converted_launch_v1(
...
{
    if (!function || !parameters || extra || runtime_launch_in_progress)
        return CUDA_ERROR_NOT_SUPPORTED;
    using direct_type = CUresult (*)(CUfunction, unsigned int, unsigned int,
        unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
        CUstream, void**, void**);
    auto direct = reinterpret_cast<direct_type>(dlsym(
        RTLD_DEFAULT, "bpftime_nv_strict_direct_cu_launch_kernel_v1"));
    if (!direct) return CUDA_ERROR_NOT_SUPPORTED;
    try {
        auto launch_guard = runtime_gate().launch_guard();
        FutureGeometryScope geometry(
            {{grid_x, grid_y, grid_z}, {block_x, block_y, block_z}});
        const auto decision = inspect_function_launch(function, parameters,
                                                      nullptr);
        if (!approve(decision) || approval_code(decision) != 2)
            return CUDA_ERROR_NOT_SUPPORTED;
        RuntimeLaunchScope scope(2, function);
        return direct(function, grid_x, grid_y, grid_z, block_x, block_y,
                      block_z, shared_memory, stream, parameters, nullptr);
    } catch (...) { return CUDA_ERROR_NOT_SUPPORTED; }
}
