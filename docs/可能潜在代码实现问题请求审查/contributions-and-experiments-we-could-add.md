...
sixteen experiments already agreed live in `docs/EXPERIMENTS-NEEDED.md` and are not
...
entry `11-write-charged-per-instruction-not-per-4kib-unit.md`.
...
immediately. Entry `07-same-page-reread-charged-as-a-new-page.md` has the code line and
...
| A page-0 write triggers an automatic block erase | Not expressible. Erase exists only in the detailed path, as a derived constant; see entry `09-erase-latency-is-a-derived-constant.md`. |
...
| The capacity mode's write-back | Runs, but performs an operation the device forbids. Entry `12-capacity-mode-rewrites-a-page-in-place.md`. |
...
entry `09-erase-latency-is-a-derived-constant.md`.** Writing the code without a number
...
device forbids. Entry `12-capacity-mode-rewrites-a-page-in-place.md` gives the two ways
...
sweep, comparable in size to item 14 of `docs/EXPERIMENTS-NEEDED.md`.
...
`docs/EXPERIMENTS-NEEDED.md` holds sixteen items ordered by which contribution claim each
...
| 7 | Existing item 8, split the 164.70x figure | Also needs the profile question in entry `13-nominal-profile-time-scale-100.md` answered |
...
- **Existing item 11** re-anchors the profiles to "approximately 0.4 to 3.0 TB/s per
  ...
  `spec-conformance-findings.md`, finding 6.
- **Existing item 6**, on the size of the overestimate from cache hits, overlaps new item
  3 and entry `08-reference-path-has-no-cache-read-either.md`. The specification's
...
- **Existing item 3** should state its result in units of launches unless a denominator
  route succeeds. The reasoning is in entry `10-accounting-unit-is-the-kernel-launch.md`,
  question three.

Everything else in `docs/EXPERIMENTS-NEEDED.md` stands as written, including item 13,
