...
| [hf_routing_worker.py](../../adapters/vllm_capacity/hf_routing_worker.py) | `make_protocol`, `llm_arguments`, `make_environment`, `serialize_return` | Derive strict metadata/token/route controls, prepare private caches and fixed single-GPU/no-addon startup controls, and validate a copied return array; helpers alone do not execute or authenticate capture |
...
| [hf_loaded_arm.py](../../scripts/eval/hf_loaded_arm.py) | `run_loaded_arm` | Private composition of one already-prepared owned request; frozen tuning validation, preconstruction retention and pregeneration observation, bound tuning report, raw return before unchanged collector, retained ownership/cleanup; parent/controlled-import/gold gates remain separate |
...
| [hf_owned_worker.py](../../scripts/eval/hf_owned_worker.py) | `execute_owned`, `_torch_device_gate`, `_vllm_device_gate` | Controlled child startup, exact request/environment/ownership bindings and staged device/runtime gates before the loaded arm; independent postchecks and wrapper diagnostics; CPU/MOCK controls do not authenticate an actual GPU run |
| [hf_routing_runner.py](../../scripts/eval/hf_routing_runner.py) | `run_triplet`, `_ownership_callback`, `_validate_owned_bootstrap` | Parent CPU orchestration of three fresh sequential processes under one guard, immutable acknowledgement/bootstrap checks and attempted-arm cleanup evidence; provisional unvalidated status only |
