...
def prepare(root,point,generated,trace,step,mesh,policy,family,deps,reason='',binary=None,model_lock=None,reference_environment=None,
            code_root=None,campaign_path=None,authorization_path=None,base_plan=None,preflight=None,regression_evidence=None,diagnostic_domain_version=None):
    root=root.resolve()
    def local(value,default):
        p=Path(value) if value is not None else Path(default);p=(p if p.is_absolute() else root/p).resolve()
        if p!=root and root not in p.parents:raise ValueError('path escapes task root')
        return p
    code=local(code_root,'eq3_thermal/worktree');campaign=local(campaign_path,'eq3_thermal/plans/campaign-v1');plan=campaign/'points'/point
    auth_path=local(authorization_path,campaign/'authorization-v3.json')
    if not code.is_dir() or not campaign.is_dir() or not auth_path.is_file():raise ValueError('code, campaign, or authorization path unavailable')
    auth=json.loads(auth_path.read_text());scope=json.loads(local(auth['scope_path'],auth['scope_path']).read_text());g=generated.resolve()
    if root not in g.parents or not g.is_dir():raise ValueError('generated input path outside task root or unavailable')
    r=json.loads((g/'generation_receipt.json').read_text())
    base=local(base_plan,'eq3_thermal/plans/layered-R02-v1');m=json.loads((base/'experiment_manifest.json').read_text());l=json.loads((base/'launch.json').read_text())
    ...
    if family in ('rc','rc_pilot'):
        ...
        if diagnostic_domain_version is not None:
            l['command']['argv'] += ['--model-sha256',sha(g/'model.txt'),'--events-sha256',sha(g/'events.txt'),'--runner-source-sha256',sha(code/'tools/eq3_campaign_rc_runner.cpp'),'--domain-version',diagnostic_domain_version]
    elif policy in ('lossless_gzip','lossless_EQ3TMK1'):
        l['backend']=art(code/'tools/eq3_campaign_stream.py','byte-lossless output adapter','stream-backend');nx,ny,nz=r['reference_shape'];l['command']['argv']=[l['backend']['path'],'--backend',reference_engine['path'],'--stack','package.stk','--layers',str(nz),'--frames',str(round(r['duration_s']/step)),'--nx',str(nx),'--ny',str(ny),'--watchdog',str(scope['limits'].get('watchdog_s',600)),'--process-ram-gib',str(scope['limits'].get('process_ram_gib',12)),'--max-output-gib',str(scope['limits'].get('point_disk_gib',4))]
    ...
    if scope.get('resource_policy')=='PER_EXPERIMENT_USER_CONFIRMED':
        limits=scope['limits'];l['limits'].update(threads=limits['threads'],ram_gib=limits['process_ram_gib'],watchdog_seconds=limits['watchdog_s'],gpu_compute_minutes=0)
        l['output']['max_new_gib']=limits['point_disk_gib']
        requested=m['resource_budget']['requested'];requested.update(build_threads=limits['threads'],ram_gib=limits['process_ram_gib'],disk_gib=limits['point_disk_gib'],gpu_compute_minutes=0,cpu_configurations=1,executions_per_configuration=1)
        declared=m['resource_budget']['limits'];declared.update(build_threads=limits['threads'],ram_gib=limits['process_ram_gib'],disk_gib=limits['point_disk_gib'],gpu_compute_minutes=0,cpu_configurations=1,executions_per_configuration=1)
    write('launch.json',l)
    ...
    def source_relative(item):
        parts=Path(item['path']).parts
        if 'worktree' not in parts:raise ValueError('base source artifact has no portable checkout-relative path')
        return Path(*parts[parts.index('worktree')+1:])
    m['code']={'repository_revision':rev,'dirty_diff_sha256':diff,'artifacts':[art(code/source_relative(p),'source') for p in m['code']['artifacts']]}
    ...
    preflight_path=local(preflight,campaign/'FOLLOWUP_DIAGNOSTIC_PREFLIGHT.md')
    if preflight is not None and not preflight_path.is_file():raise ValueError('explicit preflight evidence unavailable')
    if preflight_path.is_file():m['dependencies'].append(art(preflight_path,'stage-derived diagnostic preflight'))
    if family in ('rc','rc_pilot'):
    ...
    regression_path=local(regression_evidence,campaign/'software-tests-v6.log')
    if regression_evidence is not None and not regression_path.is_file():raise ValueError('explicit regression evidence unavailable')
    if regression_path.is_file():
        m['prerequisites']=[m['prerequisites'][0],{'id':'campaign-regression','status':'PASSED','evidence':art(regression_path,'fixed regression raw evidence')}]
