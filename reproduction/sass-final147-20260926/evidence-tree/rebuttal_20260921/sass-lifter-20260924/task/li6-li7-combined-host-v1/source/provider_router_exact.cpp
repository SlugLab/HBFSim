...
bool combined_mode() {
  static const bool enabled = [] {
    const char* value = std::getenv("HBFSIM_QKV_COMBINED_V1");
    return value && std::strcmp(value, "1") == 0;
  }();
  return enabled;
}
bool exact_combined_symbol(const char* name) {
  return name && (std::strcmp(name,kQkvSymbol)==0 ||
                  std::strcmp(name,kLi7Symbol)==0);
}
bool selected_or_combined_symbol(const char* name) {
  return combined_mode() ? exact_combined_symbol(name) :
         name && std::strcmp(name,selected_symbol())==0;
}
...
std::string exact_qkv_api_fields(const CUpti_CallbackData* d,CUpti_CallbackId id){
  if(!d||!selected_or_combined_symbol(d->symbolName))return {};
...
void capture_qkv_launch(const CUpti_CallbackData* d, CUpti_CallbackId id, CUfunction f) {
  if (!inside_qkv_blas || !d || !d->symbolName ||
      !selected_or_combined_symbol(d->symbolName)) return;
...
extern "C" int hbfsim_qkv_live_identity_for_entry_v1(
    std::uint64_t function, std::uint64_t expected_context,
    const char* expected_exact_symbol, QkvLiveIdentityV1* out) {
  if (!out || out->struct_size != sizeof(QkvLiveIdentityV1) ||
      !function || !expected_context || !exact_combined_symbol(expected_exact_symbol))
    return -1;
  std::memset(out, 0, sizeof(*out));
  out->struct_size = sizeof(*out);
  std::lock_guard<std::mutex> g(state_mu());
  const auto* a = qkv_live_associations().find(function);
  if (!a || a->context != expected_context ||
      a->name != expected_exact_symbol) return 0;
  const auto module = modules().find(a->module);
  const auto func = functions().find(function);
  const auto fm = function_modules().find(function);
  const auto parent = module_parents().find(a->module);
...
// CPU fixture hooks follow.
extern "C" const char* hbfsim_qkv_test_callback_fields(const char* name,
                                                        int callback_id) {
  static thread_local std::string fields;
  CUpti_CallbackData data{};
  data.symbolName = name;
  fields = exact_qkv_api_fields(&data,
      static_cast<CUpti_CallbackId>(callback_id));
  return fields.c_str();
}
