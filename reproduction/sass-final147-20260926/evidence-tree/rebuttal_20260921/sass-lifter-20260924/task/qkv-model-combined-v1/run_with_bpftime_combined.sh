...
HBFSIM_ROOT=/root/hbfsim-exp/rebuttal_20260921/sass-lifter-20260924/workspace-freeze-copy-v5/B-weight-binding-config-source-v1
if [[ ! -d $HBFSIM_ROOT ]]; then
    echo "run_with_bpftime_combined: frozen copied B root missing: $HBFSIM_ROOT" >&2
    exit 66
fi
...
target_extra=${HBFSIM_TARGET_EXTRA_LD_PRELOAD:-}
if [[ -n $original_preload || $target_extra != /* || ! -r $target_extra || $target_extra == *:* ]]; then
    echo "run_with_bpftime_combined: outer preload must be empty and one readable absolute target provider is required" >&2
    exit 66
fi
...
export LD_PRELOAD="$BPFTIME_AGENT:$HBFSIM_GATE:$target_extra"
set +e
"$@"
status=$?
set -e
