...
struct FutureBudgets {
    ...
    // Zero means not computed: callers must supply actual launch topology.
...
struct FuturePlan {
    ...
    // Safe only for the declared CPU analysis subset. Emission still needs
    // a per-producer valid token: predicates can change before consumption.
    // This result does not authorize a production async launch.
