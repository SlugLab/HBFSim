...
| [hf_routing_worker.py](../../adapters/vllm_capacity/hf_routing_worker.py) | `make_protocol`, `llm_arguments`, `make_environment`, `serialize_return` | Derive strict metadata/token/route controls, prepare private cache settings, and validate a copied return array; helpers alone do not execute or authenticate capture |
...
| [hf_runtime_imports.py](../../scripts/eval/hf_runtime_imports.py) | `observe_runtime_imports` | Inspect retained loaded module origins, exact cache/log/temp paths and finite settings; avoid manager/import/cache APIs, reject live manager environment and binding replacement; passive configuration evidence with injected inputs always MOCK |
