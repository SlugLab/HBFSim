...
| HF-CHECKPOINT | READABLE — REFRESH PENDING | The project HF view `phase3/models/Qwen3-30B-A3B` resolves on the host to existing safetensors. Historical inventory is BF16, distinct from the GGUF F16 identity. Sandbox link denial is not file absence. | Verify bounded headers/config/index and file identity; retain historical payload hashes as historical evidence, without rereading weight payloads or modifying the cache. |
