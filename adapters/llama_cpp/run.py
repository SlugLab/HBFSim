...
def run_once(args: argparse.Namespace, mode: str, destination: pathlib.Path) -> dict:
    ...
    if mode == "timing":
        ...
        if args.package_thermal_stage != "off":
            if args.package_thermal_profile is None or args.package_thermal_model is None:
                raise ValueError(
                    "--package-thermal-profile and --package-thermal-model are required "
                    "for a non-off package thermal stage"
                )
            config["package_thermal_profile_path"] = str(
                args.package_thermal_profile.resolve()
            )
            config["package_thermal_model_path"] = str(
                args.package_thermal_model.resolve()
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--package-thermal-profile", type=pathlib.Path)
    parser.add_argument("--package-thermal-model", type=pathlib.Path)
