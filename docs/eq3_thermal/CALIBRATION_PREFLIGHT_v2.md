# CALIBRATION_PREFLIGHT v2：逐die研究封装

ID：EQ3-P2-PERDIE-CALIBRATION；v2；PENDING_USER_APPROVAL。
...
首轮7参考+3快速=10个起始点，目标模拟856s；跳过R05则9点/756s。
不是总体上限。后续按下述审核迭代规则登记新点，不因已执行次数停下。
...
全部串行、OMP/BLAS1；最多4总编译线程且不与实验并行。首轮CPU估计3600s，
每累计3600CPU-s复核成本/证据，不作为科学停止上限。每进程600s安全watchdog、
新输出4GiB、GPU0、云费用0。安全中止后先诊断，不自动无限续跑。参考和runs现有
约568MiB，本轮新增公开原件约11MiB；既有环境/源码另占空间，保留总新增20GiB
边界。墙钟估计10–60min仅资源规划，不是对新多层案例的实测预测；首次资源
pilot决定后续是否可行。不得影响其他vLLM服务。

三空间级别报告差异趋势而非连续解证明；区域/热点max差目标均≤0.25K。
20→10ms达到目标就停止细化，否则先10→5ms。若仍未达到且预登记的各误差差值
持续降到前次的≤80%（零值容差1e-12），可在已明确批准的自适应范围和资源安全
边界内继续更细分辨率；没有固定次数/最终步长上限，每次先记录理由、预期信息
和成本再运行。没有改善或原件/推导矛盾时暂停数值扩张，分层诊断；同一修复两次
不成功必须扩大原因审核。不是重复调参到PASS，也不允许未经批准的参数矩阵。

用户最新要求明确取消机械迭代上限：依据官方原文、推导和结果审阅修订。
每轮记录问题、证据原文位置/hash、假设、输入差异、预注册指标、负结果、累计
资源、审核决定和审批范围。当前仍PENDING_USER_APPROVAL；这条规则不是启动
批准。改变器件/几何/物性/功率域/拟合方法/控制策略时，科学版本更新并重新确认。
单纯参考分辨率迭代也须在用户已确认的自适应方法内；不能借此规避元信息展示。
达到精度/问题已回答就停止；不可辨识/收益不足则诊断或重述问题；不因“做够10次”
或“估计1小时用完”机械停止。超出已批准资源范围时仍要申请，不预设无限算力。
...
本版取代v1的次数/累计时间上限，v1保留为历史，不作为当前启动依据。
本轮申请的是上述DESIGN_FREEZE候选（含有界方法提案）；执行仍被“新几何生成器
尚未实现”和“执行版本未批准”双重阻挡。设计确认后继续独立实现和固定回归，
不启动标定；把实际执行版呈现给用户确认后，再按首轮点表与获准审核迭代规则执行到完成或科学/安全停止。
