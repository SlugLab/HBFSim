...
struct MaintenanceLedger {
    struct Record {
        hbfsim::MqsimMaintenanceRequest request;
        std::string stack;
        std::uint64_t stack_local_page{};
        std::string trigger_reason;
    };
    std::map<std::uint64_t,Record> accepted;
    std::set<std::uint64_t> completed;
};

const char* maintenance_state_name(hbfsim::MqsimMaintenanceState state)
{
    using S=hbfsim::MqsimMaintenanceState;
    switch(state) {
    case S::Due:return "DUE"; case S::Queued:return "QUEUED";
    case S::Read:return "READ"; case S::ProgramDest:return "PROGRAM_DEST";
    case S::Commit:return "COMMIT"; case S::RetireOld:return "RETIRE_OLD";
    case S::ReclaimErase:return "RECLAIM_ERASE"; case S::Done:return "DONE";
    case S::Failed:return "FAILED";
    }
    throw std::logic_error("unknown maintenance state");
}

const char* maintenance_status_name(hbfsim::MqsimMaintenanceStatus status)
{
    using S=hbfsim::MqsimMaintenanceStatus;
    switch(status) {
    case S::Committed:return "COMMITTED";
    case S::CommittedReclaimDeferred:return "COMMITTED_RECLAIM_DEFERRED";
    case S::RejectedUnsupported:return "REJECTED_UNSUPPORTED";
    case S::RejectedInvalidTarget:return "REJECTED_INVALID_TARGET";
    case S::RejectedUnmapped:return "REJECTED_UNMAPPED";
    case S::RejectedNoSpare:return "REJECTED_NO_SPARE";
    case S::FailedRead:return "FAILED_READ";
...
{
    const auto value=row.value("failure_injection",std::string("none"));
    if(value=="none")return hbfsim::MqsimMaintenanceFailurePoint::None;
    if(value=="read")return hbfsim::MqsimMaintenanceFailurePoint::Read;
    if(value=="program")return hbfsim::MqsimMaintenanceFailurePoint::Program;
    if(value=="stale_commit")return hbfsim::MqsimMaintenanceFailurePoint::StaleCommit;
    if(value=="erase")return hbfsim::MqsimMaintenanceFailurePoint::Erase;
    throw std::invalid_argument("unknown maintenance failure injection");
}
...
void send(Json reply, hbfsim::MqsimOnlineEngine& engine,
          ...
          MaintenanceLedger* maintenance=nullptr)
{
        ...
        for (const auto& event : *native) {
            ...
            for (const auto& tr : event.transactions) {
                ...
                transactions.push_back({{"transaction_id",tr.transaction_id},
                    ...
                    {"maintenance_request_id",tr.maintenance_request_id ? Json(tr.maintenance_request_id) : Json(nullptr)},
                    {"maintenance_parent_id",tr.maintenance_parent_id ? Json(tr.maintenance_parent_id) : Json(nullptr)},
    ...
    Json maintenance_events=Json::array();
    for(const auto& event:engine.take_maintenance_events()) {
        Json destination=nullptr;
        if(event.destination_channel)
            destination={{"channel",*event.destination_channel},{"chip",*event.destination_chip},
                {"die",*event.destination_die},{"plane",*event.destination_plane},
                {"block",*event.destination_block},{"page",*event.destination_page}};
        maintenance_events.push_back({{"request_id",event.request_id},{"parent_id",event.parent_id},
            {"state",maintenance_state_name(event.state)},{"time_ns",event.time_ns},
            {"transaction_id",event.transaction_id ? Json(event.transaction_id) : Json(nullptr)},
            {"source",{{"channel",event.source_channel},{"chip",event.source_chip},
                {"die",event.source_die},{"plane",event.source_plane},
                {"block",event.source_block},{"page",event.source_page}}},
            {"destination",std::move(destination)}});
    }
    reply["maintenance_events"]=std::move(maintenance_events);
    Json maintenance_completions=Json::array();
    for(const auto& completion:engine.take_maintenance_completions()) {
        if(!maintenance || !maintenance->accepted.contains(completion.request_id) ||
           !maintenance->completed.insert(completion.request_id).second)
            throw std::runtime_error("unknown or duplicate maintenance completion");
        const auto& record=maintenance->accepted.at(completion.request_id);
        maintenance_completions.push_back({{"request_id",completion.request_id},
            {"parent_id",completion.parent_id},{"status",maintenance_status_name(completion.status)},
            {"enqueue_ns",completion.enqueue_ns},{"start_ns",completion.start_ns},
            {"end_ns",completion.end_ns},{"logical_page",completion.logical_page},
            {"source_version",completion.source_version},
            {"committed_version",completion.committed_version},
            {"mapping_committed",completion.mapping_committed},
            {"source_retired",completion.source_retired},{"erase_completed",completion.erase_completed},
            {"age_reset_ns",completion.mapping_committed ? Json(completion.end_ns) : Json(nullptr)},
            {"stack",record.stack},{"stack_local_page",record.stack_local_page},
            {"trigger_reason",record.trigger_reason},{"coverage_pages",1},
            {"deadline_ns",record.request.deadline_ns ? Json(record.request.deadline_ns) : Json(nullptr)},
            {"deadline_met",!record.request.deadline_ns || completion.end_ns<=record.request.deadline_ns},
            {"data_semantics","METADATA_VERSION_VALIDITY"},
            {"transaction_ids",completion.transaction_ids}});
    }
    reply["maintenance_completions"]=std::move(maintenance_completions);
    reply["now_ns"] = engine.current_time_ns();
    reply["pending"] = engine.pending();
    reply["pending_maintenance"] = engine.pending_maintenance();
    std::cout << reply.dump() << '\n' << std::flush;
...
int main(int argc, char** argv)
{
    try {
        ...
        send({{"schema_version", 1}, {"service_source", "MQSIM_SIMULATED"}, {"provenance", "PROJECTED"},
             ...
             stack_map.enabled() ? &stack_map : nullptr,nullptr,&maintenance);
        ...
        while (std::getline(std::cin, line)) {
            ...
            if (command=="submit") {
                ...
                send({{"accepted", batch.size()},
                     ...
                     stack_map.enabled() ? &placements : nullptr,&maintenance);
                ...
                if(!stack_map.enabled())
                    throw std::invalid_argument("maintenance requires an explicit HBF stack map");
                const auto id=integer(input,"request_id");
                ...
                hbfsim::HbfRequest external{.request_id=id,.sequence=0,.arrival_ns=due,
                    .logical_address=0,.bytes=profile.page_bytes,.operation=0};
                const auto placement=stack_map.map_stack_page(
                    external,input.at("stack").get<std::string>(),local_page);
                const auto backend_page=placement.backend_page;
                const auto channel=static_cast<std::uint32_t>(backend_page%profile.channels);
                const auto die=static_cast<std::uint32_t>(
                    (backend_page/profile.channels)%profile.dies_per_channel);
                const auto plane=static_cast<std::uint32_t>(
                    (backend_page/(static_cast<std::uint64_t>(profile.channels)*profile.dies_per_channel))%
                    profile.planes_per_die);
                if(!placement.expected_channel||channel!=*placement.expected_channel)
                    throw std::logic_error("maintenance placement disagrees with configured channel group");
                hbfsim::MqsimMaintenanceRequest request{
                    .request_id=id,.parent_id=parent,.due_ns=due,.deadline_ns=deadline,
                    .logical_page=backend_page,.channel=channel,.chip=0,.die=die,.plane=plane,
                    .plane_is_exact=true,
                    .reclaim_invalid_source_block=input.value("reclaim_source_block",false),
                    .failure_point=failure_point(input)};
                const auto reason=input.value("trigger_reason",std::string("UNSPECIFIED"));
                if(reason.empty())throw std::invalid_argument("empty maintenance trigger reason");
                engine.submit_maintenance(request);
                maintenance.accepted.emplace(id,MaintenanceLedger::Record{
                    request,input.at("stack").get<std::string>(),local_page,reason});
                send({{"maintenance_accepted",1},
                      {"placement",placement_json(placement)},
                      {"target",{{"channel",channel},{"chip",0},{"die",die},{"plane",plane}}},
                      {"failure_injection",input.value("failure_injection",std::string("none"))},
                      {"trigger_reason",reason},{"coverage_pages",1},
                      {"data_semantics","METADATA_VERSION_VALIDITY"}},
                     engine,native_command_observations ? &native_events : nullptr,
                     &stack_map,&placements,&maintenance);
                ...
                if (engine.pending() || engine.pending_maintenance() ||
                    accepted.size()!=completed.size() || issued_bytes!=completed_bytes ||
                    maintenance.accepted.size()!=maintenance.completed.size())
                ...
                send({{"status", "FINISHED"}, {"issued", accepted.size()}, {"completed", completed.size()},
                      ...
                      {"maintenance_issued",maintenance.accepted.size()},
                      {"maintenance_completed",maintenance.completed.size()}}, engine,
                     native_command_observations ? &native_events : nullptr,
                     stack_map.enabled() ? &stack_map : nullptr,
                     stack_map.enabled() ? &placements : nullptr,&maintenance);
                if (native_command_observations)
