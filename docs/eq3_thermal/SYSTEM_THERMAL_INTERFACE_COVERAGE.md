...
| Window byte admission | Existing `ReadRatePolicy`, future endpoint guard adapter | Next service window | One of three explicit policies | Completed facts affect future work only; active causal transfers drain; this is not native backend latency |
...
| Compute completion | Single shared scenario compute resource in `CausalExecutor` | Token completion and dynamic GPU energy | Explicit compute durations and active power | Architecture-derived synthetic weight dependency DAG; no calibrated product token/s, KV or activation traffic claim |
