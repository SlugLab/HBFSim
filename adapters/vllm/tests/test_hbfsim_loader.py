...
def test_package_thermal_stage_is_additive_and_validated(tmp_path):
    assert config(tmp_path).package_thermal_stage == "off"
    configured = config(
        tmp_path,
        package_thermal_stage="shadow",
        package_thermal_profile_path="/profiles/package.json",
        package_thermal_model_path="/models/rom.json",
    )
    assert configured.package_thermal_stage == "shadow"
    with pytest.raises(ValueError, match="package_thermal_stage"):
        config(tmp_path, package_thermal_stage="invalid")
    with pytest.raises(ValueError, match="required together"):
        config(tmp_path, package_thermal_stage="active")
