...
| [timing_future_abi.hpp](../../include/hbfsim/timing_future_abi.hpp) | `poll_state`, `consume_state`, `commit_transition`, `accounting_delta` | Versioned kernel-local 64/32-byte state and metadata, validated per-lane transitions and trace-before-accounting settlement; C5 remains disabled |
...
| [launch_gate.cpp](../../src/cuda_runtime/launch_gate.cpp) | `read_future_requirements`, `initialize_future_control`, `require_timing_binding` | Detect explicit future requirements before trust, initialize only disabled state, and reject incomplete/opaque launches including after unload |
| [hbf_device.cu](../../src/cuda_runtime/device/hbf_device.cu) | `__hbfsim_timing_future_issue_v1`, `__hbfsim_timing_future_poll_v1`, `__hbfsim_timing_future_wait_v1` | Optional FAST scalar TIMING-read validation/group reservation/poll/consume helpers; no C5 module enablement |
| [plugin.cpp](../../src/ptxpass_hbf/plugin.cpp), [transform.cpp](../../src/ptxpass_hbf/transform.cpp) | `TrustedModuleRegistry::identity_for`, `transform_ptx` | Separate original PTX/mode/helper/contract identities and refuse incomplete public future transformation; private C6.1 emitter is separate from dispatch |
...
| [future_transform.cpp](../../src/ptxpass_hbf/future_transform.cpp) | `emit_timing_futures` | Private evaluation emitter: finite per-producer slots, executed-path issue/native-load/wait-return dependencies and drains; CTA estimates assume declared block limit, not observed launch geometry |
| [hbf_device.cu](../../src/cuda_runtime/device/hbf_device.cu) | `__hbfsim_timing_future_native_store_guard_v1` | Validate entire native-store span against every HBF range before write, including overlap/overflow; no media reservation or public admission |
